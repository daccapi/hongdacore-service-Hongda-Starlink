...
- 修复 WS 分片丢失/超时、gRPC 空包死锁/崩溃/重复建链、失效 QUIC 会话无法恢复。
- 修复节点切换后 DNS 使用旧节点、DoH 失败回退系统 DNS、DNS TTL 被延长、压缩指针循环导致栈溢出。
- 统一真实代理测速健康标准并限制全局并发；自动选择不再选取仅 TCP 可达的节点，QUIC 不再测 TCP。
- TUN 巡检 HTTPS 失败后下一周期继续验证 HTTPS；断开重连后丢弃旧一轮巡检结果。
- 扩展配置生成逐节点检查；Core 导入端口在转 uint16 前验证范围，避免端口溢出。
- 新增本地协议/DNS/并发/节点健康回归测试；验证期间不关闭 Karing，不启动 Hongda TUN。
