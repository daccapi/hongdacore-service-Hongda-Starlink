...
$SiblingName = 'Hongda-Starlink-V1.6.4-R2-Android-CoreBridge-R9-KaringCompat-Source\core\sing-box'
$SiblingCandidates = @(
    (Join-Path (Split-Path -Parent $Root) $SiblingName),
    (Join-Path (Split-Path -Parent (Split-Path -Parent $Root)) $SiblingName)
)
$CoreVersion = '1.13.18'
