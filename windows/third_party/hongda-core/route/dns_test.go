...
func TestDNSCachePersistsWithTTLAndRestoresTransactionID(t *testing.T) {
	var requests atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		requests.Add(1)
		query, _ := io.ReadAll(request.Body)
	...
	cacheDir := t.TempDir()
	firstRouter := New("direct", &telemetry.Traffic{}, telemetry.NewConnections())
	firstRouter.SetPolicy(nil, model.DNSOptions{Mode: "doh", Server: server.URL})
	firstRouter.EnableDNSPersistence(cacheDir)
	query := buildDNSQuery("persisted.example", 1)
	if _, err := firstRouter.ExchangeDNS(context.Background(), query); err != nil {
		t.Fatal(err)
	}
	firstRouter.Close()

	cachePath := filepath.Join(cacheDir, dnsCacheFileName)
	stored, err := os.ReadFile(cachePath)
	if err != nil {
		t.Fatal(err)
	}
	if bytes.Contains(stored, []byte("persisted.example")) {
		t.Fatal("persistent DNS cache leaked a plaintext domain")
	}

	secondRouter := New("direct", &telemetry.Traffic{}, telemetry.NewConnections())
	defer secondRouter.Close()
	secondRouter.SetPolicy(nil, model.DNSOptions{Mode: "doh", Server: server.URL})
	secondRouter.EnableDNSPersistence(cacheDir)
	secondQuery := append([]byte(nil), query...)
	binary.BigEndian.PutUint16(secondQuery[0:2], 0x4321)
	response, err := secondRouter.ExchangeDNS(context.Background(), secondQuery)
	if err != nil {
		t.Fatal(err)
	}
	if requests.Load() != 1 {
		t.Fatalf("DoH requests = %d, want persisted cache hit", requests.Load())
	}
	if binary.BigEndian.Uint16(response[0:2]) != 0x4321 {
		t.Fatalf("restored transaction ID = %x", response[0:2])
	}
}
