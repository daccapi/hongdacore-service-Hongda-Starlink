...
func (r *Router) matchDestination(ctx context.Context, rule model.Rule, host string) bool {
	...
	if address, err := netip.ParseAddr(host); err == nil {
		return r.matchAddress(m, address.Unmap())
	}
	if containsString(m.Domain, host) || matchDomainSuffix(m.DomainSuffix, host) ||
		matchDomainKeyword(m.DomainKeyword, host) || r.matchRuleSetDomain(m.RuleSet, host) {
		return true
	}
