...
func (v *VLESS) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	// VLESS servers are allowed to send the response header together with the
	// first bytes returned by the destination. Waiting for it inside DialContext
	// deadlocks servers that wait for the client's first payload (notably
	// Vision/REALITY). Consume it lazily on the first application read.
	responseConn := &vlessResponseConn{Conn: conn}
	if v.xudp && network == "udp" {
		...
		return newXUDPConn(responseConn, realHost, realPort)
	}
	if network == "udp" {
		return &vlessUDPConn{Conn: responseConn}, nil
	}
	if v.flow == vlessFlowVision {
		return newVlessVisionConn(responseConn, v.uuid), nil
	}
...
func (c *vlessResponseConn) Read(p []byte) (int, error) {
	c.readOnce.Do(func() { c.readErr = readVLESSResponse(c.Conn) })
	if c.readErr != nil {
		return 0, c.readErr
	}
