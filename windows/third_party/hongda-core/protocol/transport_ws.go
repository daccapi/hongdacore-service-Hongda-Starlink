...
func websocketDial(ctx context.Context, base net.Conn, host string, port uint16, transport *model.TransportOptions) (net.Conn, error) {
	...
	stopCancel := context.AfterFunc(ctx, func() { _ = base.Close() })
	defer stopCancel()
	if deadline, ok := ctx.Deadline(); ok {
		_ = base.SetDeadline(deadline)
		defer base.SetDeadline(time.Time{})
	}
	path := transport.Path
	if path == "" {
		path = "/"
...
func (w *websocketConn) readFrame() (byte, []byte, error) {
	...
	final := first&0x80 != 0
	if first&0x70 != 0 {
		return 0, nil, fmt.Errorf("websocket: unexpected extension bits")
	}
	if opcode >= 0x3 && opcode <= 0x7 || opcode > wsPong {
		return 0, nil, fmt.Errorf("websocket: reserved opcode %d", opcode)
	}
	if opcode == wsContinuation {
		if !w.fragmented {
			return 0, nil, fmt.Errorf("websocket: unexpected continuation")
		}
		w.fragmented = !final
	} else if opcode == wsBinary || opcode == wsText {
		if w.fragmented {
			return 0, nil, fmt.Errorf("websocket: interleaved data message")
		}
		w.fragmented = !final
	} else if !final {
		return 0, nil, fmt.Errorf("websocket: fragmented control frame")
	}
	...
	if length > 8*1024*1024 || opcode >= wsClose && length > 125 {
		return 0, nil, fmt.Errorf("websocket: frame too large")
	}
	var mask [4]byte
	if masked {
