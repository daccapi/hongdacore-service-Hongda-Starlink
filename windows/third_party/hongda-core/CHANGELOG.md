...
- `DELETE /connections` 改为立即返回、后台关闭已登记的 TCP/UDP 真实 socket，
  避免 TUN UDP 包循环退出较慢时 Clash API 阻塞，导致 Flutter 节点切换超时。
...
- 支持 Windows 客户端现用的五组 Karing 远程规则集；binary `.srs` URL 会读取
...
- Clash `/traffic?token=...` 支持 query token 鉴权，修复启用 API secret 后 Flutter
  实时流量 WebSocket 无法连接。
...
- 关闭 Karing 后的独立管理员实测通过：国外代理、国内 DIRECT、UDP/TCP DNS
