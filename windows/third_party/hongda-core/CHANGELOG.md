...
- HY2/TUIC 不再返回终止的缓存 QUIC 会话；并发 TCP 建链合并，Hysteria2 TCP 响应握手受 context 限制。
- DNS 连接池/缓存绑定真实节点；切换或清理连接时取消旧 DNS 请求；DoH 失败不再回退系统 DNS。
- DNS TTL 不再被抬高，缓存返回扣除已消耗时间；SOA 负缓存、OPT 标志、持久化缓存作用域和时间戳得到校验。
- DNS 名称解析改成有界迭代，拒绝压缩指针循环、超长名称和越界标签。
- 自动与手动 URLTest 共用 HEAD/不重定向/仅 2xx 健康标准，全局并发最多 4；排队不计延迟。
- 导入 server_port 在 uint16 转换前检查范围；增加协议、DNS、并发、端口回归测试。
- 本轮不启用真实 TUN，不变更系统代理；生产节点互通与速度仍待单独验证。

## 1.10.11

- Hysteria2 新增 Salamander UDP 混淆：每个 QUIC 数据报使用独立随机盐，按公开协议
  生成 BLAKE2b-256 密钥并还原/混淆负载。
- 外部配置导入器接受 `obfs.type=salamander` 与密码；缺少密码和未知混淆继续在
  Check/Doctor 阶段返回明确错误。
- 新增 Salamander 数据包往返、盐与密码隔离和配置导入测试；功能集增加
  `hysteria2-salamander`，版本升级为 `1.10.11`。

## 1.10.10

- `/proxies/{name}/delay` 保留 Clash 兼容 `delay` 总耗时，并新增
  `connectDelay` 与 `totalDelay`，可区分代理传输建立和目标 TLS/HTTP 响应。
- URLTest 请求改用 HEAD 并禁止自动跟随重定向，避免把响应正文或跳转链误计入测速。
- 新增分阶段延迟 API 回归测试；功能集增加 `delay-phase-metrics`，版本升级为
  `1.10.10`。

## 1.10.9

- DoH 消息缓存新增 TTL 持久化：后台每五秒合并写入，正常关闭时刷新；重启只恢复尚未
  过期且结构有效的响应，并继续按新查询恢复 transaction ID。
- Windows DNS 缓存使用当前用户 DPAPI 和独立 entropy 加密；解密失败、缓存损坏或
  版本不匹配均安全忽略，不阻塞 Core 启动。
- URLTest 开始应用配置中的 `tolerance`：健康当前成员在阈值内保持，当前失败或最佳
  成员具有超阈值优势时切换。
- 功能集增加 `dns-cache-persistent`、`dns-cache-dpapi`、`urltest-tolerance`，
  版本升级为 `1.10.9`。

## 1.10.8

- Clash API 配置只允许 IPv4/IPv6 回环地址或 `localhost`；非回环控制面在 Doctor/Check
  阶段直接拒绝，避免导入配置意外暴露本地控制 API。
- 远程规则集的 IPv4/IPv6 CIDR 改用共享分支的二进制前缀树；单次查找不再随
  China IP 等大型规则集条目数量线性增长。
- 新增回环地址与双栈前缀匹配回归测试；功能集增加 `loopback-api` 和
  `rule-set-prefix-trie`，版本升级为 `1.10.8`。

## 1.10.7

- `/connection-log` 新增 `after` 游标、`limit` 与返回游标；活动连接继续实时更新，
  已关闭历史只返回新增或状态发生变化的记录，避免桌面端反复传输 1000 条完整历史。
- 远程规则集支持主源与多个备用源；缓存失效时依次回退，仍优先使用最后一份有效缓存。
- 可选规则集首次安装且所有下载源不可用时以空规则集启动，让 `route.final` 接管流量，
  避免 Doctor 阶段阻断整个代理连接。
- 功能集新增 `connection-log-delta`、`rule-set-fallback`、
  `rule-set-offline-start`，版本升级为 `1.10.7`。
