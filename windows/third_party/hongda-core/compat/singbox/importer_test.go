...
func TestImportPreservesRemoteRuleSets(t *testing.T) {
	cfg, err := Import(map[string]interface{}{
		...
		"route": map[string]interface{}{
			"rule_set": []interface{}{map[string]interface{}{
				...
				"download_detour": "direct", "update_interval": "1d", "download_optional": true,
				"download_fallback_url": []interface{}{"https://backup.example/cn.srs"},
			}},
	...
	if len(cfg.Rulesets) != 1 || cfg.Rulesets[0].ID != "cn" || cfg.Rulesets[0].UpdateInterval != 86400 ||
		!cfg.Rulesets[0].Optional || len(cfg.Rulesets[0].FallbackURLs) != 1 {
...
func TestImportRejectsUnsupportedProtocolSemantics(t *testing.T) {
	tests := []struct {
		...
		{
			name: "unknown hysteria obfs",
			outbound: map[string]interface{}{
				"type": "hysteria2", "tag": "node", "server": "example.com", "server_port": float64(443), "password": "secret",
				"obfs": map[string]interface{}{"type": "unknown", "password": "obfs-secret"},
			},
			want: "hysteria2 obfs",
		},
		{
...
func TestImportHysteria2Salamander(t *testing.T) {
	cfg, err := Import(map[string]interface{}{
		"outbounds": []interface{}{map[string]interface{}{
			"type": "hysteria2", "tag": "hy2", "server": "example.com",
			"server_port": float64(443), "password": "secret",
	...
	if cfg.Nodes[0].Auth["obfs"] != "salamander" || cfg.Nodes[0].Auth["obfs_password"] != "obfs-secret" {
		t.Fatalf("hysteria2 auth = %#v", cfg.Nodes[0].Auth)
	}
}
