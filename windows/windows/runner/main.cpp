...
int APIENTRY wWinMain(_In_ HINSTANCE instance,
  ...
  const int max_width_px = std::max(760, work_width - 24);
  const int max_height_px = std::max(560, work_height - 24);
  const int min_width_px = std::min(1040, max_width_px);
  const int min_height_px = std::min(660, max_height_px);
  const int initial_width_px = std::clamp(static_cast<int>(work_width * 0.92), min_width_px, max_width_px);
  const int initial_height_px = std::clamp(static_cast<int>(work_height * 0.94), min_height_px, max_height_px);
  ...
  if (!window.Create(L"鸿达星轨智连 V1.6.25", origin, size)) {
