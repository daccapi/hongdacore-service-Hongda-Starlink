...
std::wstring g_proxy_managed_bypass;
...
std::optional<std::string> UnprotectLocalData(
    const std::vector<uint8_t>& value, DWORD* error_code) {
  if (value.empty()) {
...
// Manual Windows proxies use either "host:port" or a list such as
// "http=host:port;https=host:port;socks=host:port". Only localhost proxies
// are checked here; a remote/corporate proxy is always restored unchanged.
bool ExtractLoopbackProxyPorts(const std::wstring& proxy_server,
                               std::set<USHORT>* ports,
                               bool* has_remote_endpoint) {
  if (!ports || !has_remote_endpoint) return false;
  ports->clear();
  *has_remote_endpoint = false;
  bool parsed_any = false;
  size_t start = 0;
  while (start <= proxy_server.size()) {
    const size_t end = proxy_server.find(L';', start);
    std::wstring endpoint = Trim(proxy_server.substr(
        start, end == std::wstring::npos ? std::wstring::npos : end - start));
    start = end == std::wstring::npos ? proxy_server.size() + 1 : end + 1;
    if (endpoint.empty()) continue;

    const size_t assignment = endpoint.find(L'=');
    if (assignment != std::wstring::npos) {
      endpoint = Trim(endpoint.substr(assignment + 1));
    }
    const size_t scheme = endpoint.find(L"://");
    if (scheme != std::wstring::npos) endpoint = endpoint.substr(scheme + 3);
    const size_t path = endpoint.find(L'/');
    if (path != std::wstring::npos) endpoint.resize(path);

    std::wstring host;
    std::wstring port_text;
    if (!endpoint.empty() && endpoint.front() == L'[') {
      const size_t close = endpoint.find(L']');
      if (close == std::wstring::npos || close + 2 > endpoint.size() ||
          endpoint[close + 1] != L':') {
        return false;
      }
    ...
    wchar_t* number_end = nullptr;
    const unsigned long number = wcstoul(port_text.c_str(), &number_end, 10);
    if (port_text.empty() || number_end == port_text.c_str() ||
        *number_end != L'\0' || number == 0 || number > 65535) {
      return false;
    }
    parsed_any = true;
    const bool loopback = host == L"localhost" || host == L"::1" ||
                          host == L"0:0:0:0:0:0:0:1" ||
                          host.rfind(L"127.", 0) == 0;
    if (loopback) {
      ports->insert(static_cast<USHORT>(number));
    } else {
...
bool IsTcpPortListening(USHORT port) {
  DWORD size = 0;
  ...
  if (GetExtendedTcpTable(nullptr, &size, FALSE, AF_INET6,
    ...
    if (GetExtendedTcpTable(buffer.data(), &size, FALSE, AF_INET6,
                            TCP_TABLE_OWNER_PID_LISTENER, 0) == NO_ERROR) {
      const DWORD count = *reinterpret_cast<const DWORD*>(buffer.data());
      const auto* rows = reinterpret_cast<const Tcp6RowOwnerPidCompat*>(
          buffer.data() + sizeof(DWORD));
      for (DWORD i = 0; i < count; ++i) {
        if (ntohs(static_cast<u_short>(rows[i].local_port)) == port) {
          return true;
        }
      }
    }
...
bool CanRestoreEnabledProxy(bool previous_enabled,
                            const std::wstring& previous_server) {
  if (!previous_enabled) return false;
  std::set<USHORT> loopback_ports;
  bool has_remote_endpoint = false;
  if (!ExtractLoopbackProxyPorts(previous_server, &loopback_ports,
                                 &has_remote_endpoint)) {
    return true;  // Preserve unknown Windows proxy syntax.
  }
  if (has_remote_endpoint || loopback_ports.empty()) return true;
  return std::all_of(loopback_ports.begin(), loopback_ports.end(),
                     [](USHORT port) { return IsTcpPortListening(port); });
}
...
bool SaveProxyRecovery(bool previous_enabled,
                       ...
                       const std::wstring& managed_bypass) {
  HKEY key = nullptr;
  DWORD disposition = 0;
  ...
  bool ok = SetDwordValue(key, L"Managed", 1) &&
            ...
            SetStringValue(key, L"ManagedBypass", managed_bypass);
  RegCloseKey(key);
  return ok;
}
...
bool LoadProxyRecovery(bool* previous_enabled,
  ...
  if (ok) {
    ...
    if (managed_bypass) *managed_bypass = ReadStringValue(key, L"ManagedBypass");
  }
  RegCloseKey(key);
  return ok;
}
  ...
  if (suspend) {
    if (current_enabled != 0) {
      ...
      const bool restore_enabled =
          CanRestoreEnabledProxy(true, previous_server);
      ok = SaveTunProxyRecovery(restore_enabled, previous_server,
                                previous_bypass);
      if (ok) {
    ...
    if (current_enabled == 0) {
      ...
      const std::wstring current_bypass = ReadStringValue(key, L"ProxyOverride");
      // Suspending only changes ProxyEnable. If another client edited the
      // disabled values while TUN was running, those edits win.
      if (current_server == previous_server &&
          current_bypass == previous_bypass) {
        const DWORD restored =
            CanRestoreEnabledProxy(previous_enabled, previous_server) ? 1 : 0;
        ok = RegSetValueExW(key, L"ProxyEnable", 0, REG_DWORD,
...
bool SetSystemProxy(bool enabled, const std::wstring& server,
  ...
  if (enabled && !g_proxy_managed) {
    ...
    g_proxy_previous_enabled = CanRestoreEnabledProxy(
        g_proxy_previous_enabled, g_proxy_previous_server);
    if (!SaveProxyRecovery(g_proxy_previous_enabled, g_proxy_previous_server,
                           g_proxy_previous_bypass, server, bypass)) {
      RegCloseKey(key);
      return false;
    }
    g_proxy_managed_server = server;
    g_proxy_managed_bypass = bypass;
  ...
  if (!enabled && !g_proxy_managed) {
    ...
    if (LoadProxyRecovery(&previous_enabled, &previous_server, &previous_bypass,
      ...
      if (current_enabled != 0 && !managed_server.empty() &&
          ...
          (managed_bypass.empty() || current_bypass == managed_bypass)) {
        g_proxy_previous_enabled =
            CanRestoreEnabledProxy(previous_enabled, previous_server);
        g_proxy_previous_server = previous_server;
        g_proxy_previous_bypass = previous_bypass;
        g_proxy_managed_server = managed_server;
        g_proxy_managed_bypass = managed_bypass;
        g_proxy_managed = true;
      } else {
        ClearProxyRecovery();
        RegCloseKey(key);
  ...
  if (!enabled && g_proxy_managed) {
    ...
    const std::wstring current_server = ReadStringValue(key, L"ProxyServer");
    const std::wstring current_bypass = ReadStringValue(key, L"ProxyOverride");
    if (current_enabled == 0 || current_server != g_proxy_managed_server ||
        (!g_proxy_managed_bypass.empty() &&
         current_bypass != g_proxy_managed_bypass)) {
      // A user or another proxy client changed the complete state while
      // Hongda was running. Keep that state and discard only our snapshot.
      ClearProxyRecovery();
      g_proxy_managed = false;
      g_proxy_managed_server.clear();
      g_proxy_managed_bypass.clear();
      g_proxy_previous_enabled = false;
...
bool FlutterWindow::OnCreate() {
  ...
  channel_->SetMethodCallHandler(
      [this](const flutter::MethodCall<flutter::EncodableValue>& call,
        ...
        if (call.method_name() == "protectData") {
          const auto* value =
              std::get_if<std::string>(call.arguments());
          if (!value) {
            result->Error("bad_args", "Expected UTF-8 string");
            return;
          }
          DWORD error_code = ERROR_SUCCESS;
          auto protected_data = ProtectLocalData(*value, &error_code);
          if (!protected_data) {
            result->Error(
                "dpapi_protect_failed", "CryptProtectData failed",
                flutter::EncodableValue(static_cast<int64_t>(error_code)));
            return;
          }
          result->Success(flutter::EncodableValue(*protected_data));
          return;
        }

        if (call.method_name() == "unprotectData") {
          const auto* value =
              std::get_if<std::vector<uint8_t>>(call.arguments());
          if (!value) {
            result->Error("bad_args", "Expected byte array");
            return;
          }
          DWORD error_code = ERROR_SUCCESS;
          auto plaintext = UnprotectLocalData(*value, &error_code);
          if (!plaintext) {
            result->Error(
                "dpapi_unprotect_failed", "CryptUnprotectData failed",
                flutter::EncodableValue(static_cast<int64_t>(error_code)));
            return;
          }
...
LRESULT FlutterWindow::MessageHandler(HWND hwnd, UINT const message,
  ...
  switch (message) {
    case WM_CLOSE:
      ...
      break;
	case WM_SIZE:
	  NotifyWindowVisibility(wparam != SIZE_MINIMIZED && IsWindowVisible(hwnd));
	  break;
