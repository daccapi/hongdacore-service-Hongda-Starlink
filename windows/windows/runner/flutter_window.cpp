...
flutter::EncodableMap InspectTunRouting() {
  ...
  if (adapter_found) {
    ...
    // 8.8.8.8 is byte-order invariant, so this constant avoids the deprecated
    // inet_addr API while remaining in the network order GetBestInterface uses.
    const IPAddr probe_address = 0x08080808UL;
    effective_route =
        GetBestInterface(probe_address, &best_if_index) == NO_ERROR &&
        best_if_index == tun_if_index;
  }

  const bool route_ready = default_route || split_route_count >= 2;
