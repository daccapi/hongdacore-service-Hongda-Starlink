...
enum NodeProbeStatus { untested, tcpReachable, proxyAvailable, failed }
...
class NodeProfile {
  NodeProfile({
    ...
    this.source = 'manual',
    this.latencyMs,
    this.proxyConnectDelayMs,
    this.proxyTotalDelayMs,
    this.lastLatencyTest,
    this.testError,
    this.probeStatus = NodeProbeStatus.untested,
    this.regionCode,
    this.regionName,
    this.regionSource,
    this.regionLookupAt,
    this.regionLookupError,
    this.favorite = false,
    this.enabled = true,
  });
  ...
  final String source;
  int? latencyMs;
  int? proxyConnectDelayMs;
  int? proxyTotalDelayMs;
  DateTime? lastLatencyTest;
  String? testError;
  NodeProbeStatus probeStatus;
  String? regionCode;
  String? regionName;
  String? regionSource;
  DateTime? regionLookupAt;
  String? regionLookupError;
  bool favorite;
  bool enabled;
  ...
  Map<String, dynamic> toJson() => <String, dynamic>{
    ...
    'source': source,
    'latencyMs': latencyMs,
    'proxyConnectDelayMs': proxyConnectDelayMs,
    'proxyTotalDelayMs': proxyTotalDelayMs,
    'lastLatencyTest': lastLatencyTest?.toIso8601String(),
    'testError': testError,
    'probeStatus': probeStatus.name,
    'regionCode': regionCode,
    'regionName': regionName,
    'regionSource': regionSource,
    'regionLookupAt': regionLookupAt?.toIso8601String(),
    'regionLookupError': regionLookupError,
    'favorite': favorite,
    'enabled': enabled,
  };

  factory NodeProfile.fromJson(Map<String, dynamic> json) {
    var latency = json['latencyMs'] == null
        ? null
        : _intValue(json['latencyMs']);
    var error = json['testError']?.toString();
    final rawStatus = json['probeStatus']?.toString();
    var probeStatus = switch (rawStatus) {
      'tcpReachable' => NodeProbeStatus.tcpReachable,
      'proxyAvailable' => NodeProbeStatus.proxyAvailable,
      'failed' => NodeProbeStatus.failed,
      _ => NodeProbeStatus.untested,
    };

    // V1.6.5 stored a successful TCP reachability probe in testError, which
    // rendered a red failure icon for nodes that were actually reachable.
    // Migrate that data without pretending the TCP time is a proxy handshake.
    if (error != null && error.startsWith('TCP 端口可达')) {
      final migratedLatency = RegExp(r'（(\d+) ms）').firstMatch(error);
      latency ??= int.tryParse(migratedLatency?.group(1) ?? '');
      error = null;
      probeStatus = NodeProbeStatus.tcpReachable;
    } else if (rawStatus == null) {
      if (error != null && error.isNotEmpty) {
        probeStatus = NodeProbeStatus.failed;
      } else if (latency != null && latency > 0) {
        probeStatus = NodeProbeStatus.proxyAvailable;
      }
    }

    final rawRegionCode = json['regionCode']?.toString().trim().toUpperCase();
    return NodeProfile(
      id: json['id']?.toString() ?? '',
      ...
      latencyMs: latency,
      proxyConnectDelayMs: json['proxyConnectDelayMs'] == null
          ? null
          : _intValue(json['proxyConnectDelayMs']),
      proxyTotalDelayMs: json['proxyTotalDelayMs'] == null
          ? null
          : _intValue(json['proxyTotalDelayMs']),
      ...
      regionCode:
          rawRegionCode != null && RegExp(r'^[A-Z]{2}$').hasMatch(rawRegionCode)
          ? rawRegionCode
          : null,
      regionName: json['regionName']?.toString().trim(),
      regionSource: json['regionSource']?.toString(),
      regionLookupAt: DateTime.tryParse(
        json['regionLookupAt']?.toString() ?? '',
      ),
      regionLookupError: json['regionLookupError']?.toString(),
...
class SubscriptionProfile {
  ...
  factory SubscriptionProfile.fromJson(Map<String, dynamic> json) {
    return SubscriptionProfile(
      ...
      allowInsecureHttp: json['allowInsecureHttp'] == true,
    );
  }
...
class AppSettings {
  AppSettings({
    ...
    this.windowsTunProfileVersion = 6,
    this.dnsMode = 'doh',
    ...
    if (tunEnabled) systemProxyEnabled = false;
  }
  ...
  Map<String, dynamic> toJson() => <String, dynamic>{
    ...
    'windowsTunProfileVersion': windowsTunProfileVersion,
    'dnsMode': dnsMode,
  ...
  factory AppSettings.fromJson(Map<String, dynamic> json) {
    return AppSettings(
      ...
      tunStack: json['tunStack']?.toString() ?? 'mixed',
      tunMtu: int.tryParse(json['tunMtu']?.toString() ?? '') ?? 4064,
      ipv6Enabled: json['ipv6Enabled'] == true,
      windowsTunProfileVersion:
          int.tryParse(json['windowsTunProfileVersion']?.toString() ?? '') ?? 0,
      dnsMode: json['dnsMode']?.toString() ?? 'doh',
      dnsPrimary: json['dnsPrimary']?.toString() ?? 'https://1.1.1.1/dns-query',
      dnsDirect: json['dnsDirect']?.toString() ?? 'local',
      dnsStrategy: json['dnsStrategy']?.toString() ?? 'ipv4_only',
