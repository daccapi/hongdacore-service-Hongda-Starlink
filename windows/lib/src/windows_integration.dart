...
class WindowsIntegration {
  ...
  static void Function(bool visible)? _visibilityListener;
  static bool _visibilityHandlerInstalled = false;

  static void setWindowVisibilityListener(
    void Function(bool visible)? listener,
  ) {
    ...
    _channel.setMethodCallHandler((call) async {
      if (call.method == 'windowVisibilityChanged') {
        _visibilityListener?.call(call.arguments == true);
      }
    });
  }
  ...
  static Future<String> protectData(String value) async {
    if (!Platform.isWindows) return base64Encode(utf8.encode(value));
    final protected = await _channel.invokeMethod<Uint8List>(
      'protectData',
      value,
    );
    if (protected == null || protected.isEmpty) {
      throw StateError('Windows DPAPI 没有返回加密数据');
    }
    return base64Encode(protected);
  }

  static Future<String> unprotectData(String value) async {
    final encoded = base64Decode(value);
    if (!Platform.isWindows) return utf8.decode(encoded);
    final plaintext = await _channel.invokeMethod<String>(
      'unprotectData',
      Uint8List.fromList(encoded),
    );
    if (plaintext == null) throw StateError('Windows DPAPI 解密失败');
    return plaintext;
  }
