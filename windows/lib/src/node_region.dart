...
NodeRegion nodeRegion(NodeProfile node) {
  try {
    ...
    // suffix (jpda, hk01, uspro). The previous word-boundary-only rules missed
    // these names. Limit the fallback to short tokens beginning after a real
    // separator so ordinary words such as "vless" are never classified.
    final tokens = raw
        .toLowerCase()
