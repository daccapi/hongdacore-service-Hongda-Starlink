    ...
    try {
      ...
      if (serviceBytes < 40 * 1024 * 1024) {
        throw ServiceStartException(
          ...
          detail:
              '当前只有 ${(serviceBytes / 1024 / 1024).toStringAsFixed(1)} MB；V1.6.5 自包含版本应包含内嵌 HongdaCore。',
        );
      }
      _appendLog(
      ...
      _setStartup(
        ...
        '验证 HongdaService、HongdaCore 1.13.18 与必要功能',
      );
      ...
      _setStartup(
        ...
        '首次启动会校验并释放内嵌 HongdaCore 1.13.18',
      );
        ...
        (data) {
          try {
            ...
            if (json is Map) {
              ...
              final websocketUpload = (json['up'] as num?)?.toDouble() ?? 0;
              final websocketDownload = (json['down'] as num?)?.toDouble() ?? 0;
              final keepFallback = websocketUpload == 0 &&
                  websocketDownload == 0 &&
                  _lastFallbackTrafficAt != null &&
                  now.difference(_lastFallbackTrafficAt!) < const Duration(seconds: 3);
              if (!keepFallback) {
        ...
        if (previousAt != null) {
          ...
          if (seconds > 0 &&
            ...
            if (uploadDelta > 0 || downloadDelta > 0) {
              ...
              if (!_trafficFallbackLogged) {
                _trafficFallbackLogged = true;
                _appendLog('实时流量已切换到 Clash connections 统计');
              }
            }
          }
        }
