  ...
  Future<void> _selectProxyTag(AppSettings settings, String tag) async {
    ...
    try {
      ...
      if (delayProbe.succeeded) {
        _appendLog(
          'Selector 确认：$tag · ${delayProbe.delayMs}ms · $connectionMessage',
        );
      } else {
        _appendLog(
  ...
  Future<void> _runTunWatchdog(AppSettings settings) async {
    ...
    try {
      ...
      if (!route.ready) {
        ...
        _tunWatchdogProbeCycle++;
        if (_tunWatchdogProbeCycle >= 10) {
          _tunWatchdogProbeCycle = 0;
    ...
    if (!isRunning || !settings.tunEnabled) return;
