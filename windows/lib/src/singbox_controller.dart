...
class SingBoxController extends ChangeNotifier {
    ...
    try {
      ...
      if (serviceBytes < 40 * 1024 * 1024) {
        throw ServiceStartException(
          ...
          detail:
              '当前只有 ${(serviceBytes / 1024 / 1024).toStringAsFixed(1)} MB；V1.6.6 自包含版本应包含内嵌 HongdaCore。',
        );
      }
      _appendLog(
      ...
      if (settings.tunEnabled) {
        _setStartup(StartupStage.waitingCore, '验证 HongdaTun 网卡与系统路由');
        await _verifyTunRouting();
      }
  ...
  Future<void> _verifyTunRouting() async {
    ...
    for (var attempt = 0; attempt < 20; attempt++) {
      try {
        ...
        notifyListeners();
        if (latest.ready) {
          _appendLog('TUN 路由验证通过：${latest.detail}');
          return;
        }
      } catch (error) {
