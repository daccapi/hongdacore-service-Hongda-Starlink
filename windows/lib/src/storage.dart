...
class AppStorage {
  ...
  factory AppStorage.forDirectory(Directory baseDir) => AppStorage._(baseDir);
  ...
  Future<List<SubscriptionProfile>> loadSubscriptions() async {
    try {
      final decoded =
          await _readJsonWithRecovery(subscriptionsFile) as List<dynamic>;
      return decoded
          .whereType<Map>()
          .map(
            (Map item) =>
  ...
  Future<void> saveNodes(List<NodeProfile> nodes) async {
    await _atomicWrite(
      nodesFile,
      prettyJson(nodes.map((e) => e.toJson()).toList()),
    );
  }
