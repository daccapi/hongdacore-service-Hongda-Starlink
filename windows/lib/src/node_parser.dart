...
class NodeParser {
  ...
  static NodeProfile? _parseShadowsocks(String input, String source) {
    try {
      ...
      // SIP002 shadowsocks URIs base64-encode the password inside userinfo.
      // Decode when it is valid base64/UTF-8, otherwise keep the raw value so
      // providers that already send plaintext passwords continue to work.
      final password = _decodeSsPassword(userInfo.substring(split + 1));
  ...
  static String _decodeSsPassword(String raw) {
    if (raw.isEmpty) return raw;
    try {
      final decoded = _decodeBase64(raw);
      if (decoded.isNotEmpty) return decoded;
    } catch (_) {
      // Not valid base64 or not valid UTF-8: treat the value as plaintext.
    }
    return raw;
  }
