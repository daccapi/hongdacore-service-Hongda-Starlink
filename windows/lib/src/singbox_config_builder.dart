...
class SingBoxConfigBuilder {
  ...
  static bool supportsNode(NodeProfile node) =>
      unsupportedNodeReason(node) == null;

  /// Returns the first HongdaCore compatibility problem for [node].
  ///
  /// Subscriptions can contain nodes that use options which the current Core
  /// does not implement. Keeping this check in the config builder prevents an
  /// unselected subscription node from invalidating an otherwise usable
  /// selected node during `doctor`.
  static String? unsupportedNodeReason(NodeProfile node) {
    final protocol = node.protocol.trim().toLowerCase();
    if (!supportedProtocols.contains(protocol)) {
      return '暂不支持 ${node.protocol} 协议';
    }

    final outbound = node.outbound;
    if (outbound['type'] != protocol) return '节点协议与出站类型不一致';
    final server = outbound['server'];
    if (server is! String ||
        server.trim().isEmpty ||
        server.trim() != server ||
        RegExp(r'\s').hasMatch(server)) {
      return '节点服务器地址无效';
    }
    final port = outbound['server_port'];
    if (port is! int || port < 1 || port > 65535) return '节点端口必须为 1–65535';
    if (protocol == 'vless' || protocol == 'tuic') {
      final uuid = (outbound['uuid'] ?? '').toString().trim().replaceAll(
        '-',
        '',
      );
      if (!RegExp(r'^[0-9a-fA-F]{32}$').hasMatch(uuid)) return '节点 UUID 无效';
    }
    if (protocol != 'vless' &&
        (outbound['password'] is! String ||
            (outbound['password'] as String).isEmpty)) {
      return '节点缺少代理密码';
    }
    final multiplex = outbound['multiplex'];
    if (multiplex is Map && multiplex['enabled'] == true) {
      return '暂不支持 multiplex 复用';
    }

    final transport = outbound['transport'];
    if (transport is Map) {
      final transportType = (transport['type'] ?? '')
          .toString()
          .trim()
          .toLowerCase();
      if (!const <String>{
    ...
    if (protocol == 'hysteria2') {
      ...
      if (obfs is Map) {
        final obfsType = (obfs['type'] ?? '').toString().trim().toLowerCase();
        if (obfsType.isNotEmpty && obfsType != 'salamander') {
          return '暂不支持 Hysteria2 $obfsType 混淆';
        }
        if (obfsType == 'salamander' &&
            (obfs['password'] ?? '').toString().trim().isEmpty) {
          return 'Hysteria2 Salamander 缺少混淆密码';
        }
      } else if (obfs != null && obfs.toString().trim().isNotEmpty) {
        return 'Hysteria2 混淆配置格式无效';
      }
    }

    if (protocol == 'tuic') {
      final congestionControl = (outbound['congestion_control'] ?? '')
          .toString()
          .trim()
          .toLowerCase();
      if (congestionControl.isNotEmpty && congestionControl != 'cubic') {
        return 'TUIC 暂不支持 $congestionControl 拥塞控制';
      }
      if (outbound['zero_rtt_handshake'] == true) {
        return 'TUIC 暂不支持 0-RTT';
      }
      final udpRelayMode = (outbound['udp_relay_mode'] ?? '')
          .toString()
          .trim()
          .toLowerCase();
      if (udpRelayMode.isNotEmpty && udpRelayMode != 'native') {
        return 'TUIC 暂不支持 $udpRelayMode UDP 中继模式';
      }
    }
  ...
  Map<String, dynamic> build({
    ...
    final enabledNodes = nodes
        ...
        .toList();
    final selectedUnsupportedReason = unsupportedNodeReason(selectedNode);
    if (selectedUnsupportedReason != null) {
      throw UnsupportedError('HongdaCore 1.10：$selectedUnsupportedReason');
    }
