  ...
  Map<String, dynamic> build({
    ...
    if (settings.routeMode == 'rule') {
      ...
      if (settings.smartProxyList) {
        ...
        // Keep critical OpenAI domains local to the generated config. The
        // larger lists also contain most of them, but startup must not depend
        // on a separate tiny remote rule set being available.
        routeRules.add(<String, dynamic>{
      ...
      if (settings.smartCnDirect) {
        routeRuleSets.addAll(<Map<String, dynamic>>[
          ...
          if (settings.ipv6Enabled)
            _remoteRuleSet('karing-cn-ipv6', 'ChinaIpV6.srs'),
        ]);
      ...
      if (settings.smartCnDirect) {
        ...
        routeRules.add(<String, dynamic>{
          'rule_set': <String>[
            'karing-cn-ip',
            if (settings.ipv6Enabled) 'karing-cn-ipv6',
          ],
          'action': 'route',
          'outbound': 'direct',
        });
  ...
  String _finalOutbound(AppSettings settings, Set<String> effectiveGroupIds) {
    switch (settings.routeMode) {
      ...
      case 'global':
        return 'proxy';
      case 'rule':
        // Karing-style selective routing: explicit proxy rule sets use the
        // selected node; unmatched destinations stay on the physical network.
        return 'direct';
      default:
        return 'proxy';
    }
