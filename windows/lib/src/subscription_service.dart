...
class SubscriptionService {
  static Future<SubscriptionFetchResult> fetch(
    ...
    if (uri.scheme == 'http' && !subscription.allowInsecureHttp) {
      throw const FormatException('订阅 URL 必须使用 HTTPS；如确需 HTTP，请在添加时明确允许不安全连接');
    }

    final client = HttpClient()
      ..connectionTimeout = const Duration(seconds: 15)
      ..userAgent = subscription.userAgent.trim().isEmpty
          ? 'HongdaStarlink/1.1'
          : subscription.userAgent.trim();
    try {
      ...
      if (uri.scheme == 'https' &&
        ...
        throw HttpException('拒绝 HTTPS 订阅降级重定向到 HTTP', uri: uri);
      }
  ...
  static Future<List<NodeProfile>> fetchNodes(
    SubscriptionProfile subscription,
  ) async {
  ...
  static Map<String, int> _parseUserInfo(String? header) {
    ...
    for (final part in header.split(';')) {
      ...
      if (value != null &&
          <String>{'upload', 'download', 'total', 'expire'}.contains(key)) {
        result[key] = value;
      }
    }
    return result;
