...
class AppBootstrap {
  ...
  static Future<AppBootstrap> create() async {
    ...
    if (settings.windowsTunProfileVersion < 2) {
      // Migrate the previous Windows profile (system/9000/IPv6/strict) that
      // could create a TUN adapter without reliably capturing application
      // traffic on common Windows 10/11 network stacks.
      settings.tunStack = 'mixed';
      settings.tunMtu = 1500;
      settings.ipv6Enabled = false;
      settings.strictRoute = false;
      settings.windowsTunProfileVersion = 2;
      ...
      children: <Widget>[
        _SettingsCard(
          ...
          children: <Widget>[
            _SettingSwitch(
              title: '连接后自动设置系统代理',
              subtitle: '将 Windows 当前用户代理指向 127.0.0.1:${settings.mixedPort}',
              value: settings.systemProxyEnabled,
              onChanged: (v) {
