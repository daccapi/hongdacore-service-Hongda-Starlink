...
import 'node_geolocation_service.dart';
...
class AppBootstrap {
  ...
  static Future<AppBootstrap> create() async {
    ...
    final nodes = values[1] as List<NodeProfile>;
    if (settings.windowsTunProfileVersion < 6) {
      // TUN must not be combined with the localhost Windows proxy. Some
      // AppContainer/WebView applications cannot connect to 127.0.0.1 and
      // will fail even though ordinary browsers appear healthy. Capture both
      // address families, but suppress/reject IPv6 until every route is known
      // to follow the selected node; this prevents physical-adapter leaks.
      settings.tunStack = 'mixed';
      // Karing uses a 4064-byte userspace TUN MTU on Windows. The outer proxy
      // socket is still segmented by the physical interface MTU, while the
      // larger inner MTU substantially reduces gVisor packet/syscall overhead.
      settings.tunMtu = 4064;
      settings.ipv6Enabled = false;
      settings.dnsStrategy = 'ipv4_only';
      settings.strictRoute = true;
      if (settings.tunEnabled) settings.systemProxyEnabled = false;
      settings.windowsTunProfileVersion = 6;
    ...
    if (settings.dnsMode == 'local') {
      // The previous default resolved everything with the OS resolver, which
      // leaks the user's queried domains to the ISP even while proxying.
      // Migrate to DoH-through-proxy so DNS follows the tunnel.
      settings.dnsMode = 'doh';
...
class HongdaStarlinkApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: MainShell(
        bootstrap: bootstrap,
        resumeConnectOnLaunch: resumeConnectOnLaunch,
      ),
    );
  }
}
...
class _MainShellState extends State<MainShell> with WidgetsBindingObserver {
  ...
  final NodeGeolocationService geolocation = const NodeGeolocationService();

  AppPage page = AppPage.dashboard;
  bool isAdmin = false;
  String? coreVersion;
  String? transientMessage;
  bool locatingRegions = false;
  bool _connectionActionInProgress = false;
  bool _nodeSelectionInProgress = false;
  bool _nativeWindowVisible = true;
  bool _lifecycleVisible = true;
  bool get _appVisible => _nativeWindowVisible && _lifecycleVisible;
  Timer? _autoReconnectTimer;
  int _autoReconnectAttempts = 0;
  int _handledUnexpectedExitSerial = 0;
  final List<double> uploadHistory = List<double>.filled(50, 0, growable: true);
  final List<double> downloadHistory = List<double>.filled(
    50,
    0,
    growable: true,
  );
  ...
  void initState() {
    ...
    WindowsIntegration.setWindowVisibilityListener(_onNativeWindowVisibility);
    ...
    if (widget.resumeConnectOnLaunch) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        ...
        if (!mounted ||
            ...
            controller.status == CoreStatus.starting)
          return;
    ...
    historyTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      ...
      setState(() {
        _slideSample(uploadHistory, controller.uploadBytesPerSecond);
        _slideSample(downloadHistory, controller.downloadBytesPerSecond);
      });
    });
  }
  ...
  void dispose() {
    ...
    WindowsIntegration.setWindowVisibilityListener(null);
  ...
  void _onControllerChanged() {
    ...
    if (settings.autoReconnect &&
        controller.unexpectedExitSerial > _handledUnexpectedExitSerial) {
      _handledUnexpectedExitSerial = controller.unexpectedExitSerial;
      _scheduleAutoReconnect();
    }
    if (mounted && _appVisible) setState(() {});
  }
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (_lifecycleVisible == visible) return;
    _lifecycleVisible = visible;
    if (_appVisible && mounted) setState(() {});
  }

  void _onNativeWindowVisibility(bool visible) {
    if (_nativeWindowVisible == visible) return;
    _nativeWindowVisible = visible;
    if (_appVisible && mounted) setState(() {});
  }

  void _cancelAutoReconnect({bool resetAttempts = true}) {
    _autoReconnectTimer?.cancel();
    _autoReconnectTimer = null;
    if (resetAttempts) _autoReconnectAttempts = 0;
  }
  ...
  void _scheduleAutoReconnect() {
    if (!mounted ||
        ...
        controller.isRunning ||
        controller.status == CoreStatus.starting ||
        controller.status == CoreStatus.stopping ||
        _autoReconnectTimer != null) {
      return;
    }
    if (_autoReconnectAttempts >= 3) {
      controller.recordClientEvent('自动恢复已连续失败 3 次，请检查节点或网络后手动重试', error: true);
      return;
    }
    final delaySeconds = <int>[2, 4, 8][_autoReconnectAttempts];
    controller.recordClientEvent(
      '检测到连接意外中断，${delaySeconds} 秒后执行第 ${_autoReconnectAttempts + 1}/3 次自动恢复',
    );
  ...
  Future<void> _runAutoReconnect() async {
    if (!mounted || !settings.autoReconnect || controller.isRunning) return;
    ...
    try {
      ...
      controller.recordClientEvent('连接已自动恢复');
      _autoReconnectAttempts = 0;
    } catch (_) {
      _scheduleAutoReconnect();
    } finally {
  ...
  void _slideSample(List<double> samples, double value) {
  ...
  Future<void> _detectNodeRegions({
    bool force = false,
    bool showToast = true,
  }) async {
    if (locatingRegions) return;
    if (force && showToast) {
      final approved = await showDialog<bool>(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('使用服务器 IP 检测地区'),
          content: const Text(
            '将仅把节点的服务器 IP 发送给第三方定位服务 ipwho.is，'
            '不会发送 UUID、密码、Reality 公钥或订阅地址。是否继续？',
          ),
          actions: <Widget>[
            TextButton(
    ...
    setState(() => locatingRegions = true);
    var changed = 0;
    var touched = false;
    try {
      for (final node in nodes.where((item) => item.enabled)) {
        final beforeLookup = node.regionLookupAt;
        if (await geolocation.resolve(node, force: force)) {
          changed++;
          if (mounted) setState(() {});
        }
        if (node.regionLookupAt != beforeLookup) touched = true;
      }
      if (touched || changed > 0) await storage.saveNodes(nodes);
      if (showToast && mounted) {
        _toast(
          changed > 0
              ? '已通过服务器 IP 更新 $changed 个节点的国家/地区'
              : '没有需要更新的地区；无法识别的节点可在节点菜单中手动设置',
        );
      }
    } finally {
      if (mounted) setState(() => locatingRegions = false);
    }
  }
  ...
  Future<void> _connectOrDisconnect() async {
    ...
    if (controller.status == CoreStatus.starting ||
        controller.status == CoreStatus.stopping) {
      _toast('连接操作正在进行，请稍候');
      return;
    }
  ...
  Future<void> _showServiceFailureDialog(Object error) async {
    ...
    await showGeneralDialog<void>(
      ...
      barrierLabel: '关闭启动错误',
      barrierColor: const Color(0xFF0E172A).withValues(alpha: .28),
      transitionDuration: const Duration(milliseconds: 220),
      pageBuilder: (context, animation, secondaryAnimation) =>
          _ServiceFailureDialog(
            message: message,
  ...
  Future<void> _selectNode(NodeProfile node) async {
    if (_nodeSelectionInProgress || _connectionActionInProgress) return;
    if (!node.enabled) {
      _toast('该节点已禁用，无法切换');
      return;
    }
    if (controller.status == CoreStatus.starting ||
        controller.status == CoreStatus.stopping) {
      _toast('连接状态正在变更，请稍后切换节点');
      return;
    }
    final targetTag = SingBoxConfigBuilder.nodeTag(node.id);
    if (settings.selectedNodeId == node.id &&
        (!controller.isRunning || controller.activeNodeTag == targetTag)) {
      return;
    }
    _nodeSelectionInProgress = true;
    ...
    try {
      ...
      try {
        ...
        return;
      } catch (hotSwitchError) {
        // A manually-added VLESS node can be selected while connected even if
        // the running config is stale. Fall back to a controlled core reload
        // instead of forcing the user to disconnect by hand.
        _toast('正在切换到 ${node.name}…');
        try {
          await controller.stop(settings);
          await controller.start(
            node,
          ...
          await storage.saveSettings(settings);
          if (mounted) setState(() {});
          _toast('切换节点失败：${_cleanError(reloadError)}');
        }
      }
    } finally {
      _nodeSelectionInProgress = false;
      if (mounted) setState(() {});
    }
  }
  ...
  Future<void> _autoSelectBestNode() async {
    if (!controller.isRunning) {
      _toast('请先连接，再通过真实代理测速自动选节点');
      return;
    }
    final targets = nodes
        .where(
          (node) => node.enabled && SingBoxConfigBuilder.supportsNode(node),
        )
        .toList();
    if (targets.isEmpty) {
      _toast('当前没有可用节点');
    ...
    final available = verifiedProxyCandidates(targets);
    await storage.saveNodes(nodes);
    ...
    _toast(
      '已选择代理测速最佳节点：${available.first.name} · 完整探针 ${available.first.proxyTotalDelayMs} ms',
    );
  }
    ...
    switch (page) {
      case AppPage.dashboard:
        return _DashboardPage(
          ...
          onOpenSubscriptions: () =>
              setState(() => page = AppPage.subscriptions),
      ...
      case AppPage.nodes:
        return _NodesPage(
          ...
          locatingRegions: locatingRegions,
          onDetectRegions: () => _detectNodeRegions(force: true),
        );
      ...
      case AppPage.networkTools:
        return _NetworkToolsPage(
          controller: controller,
          settings: settings,
          nodes: nodes,
          onNodesChanged: () async {
            setState(() {});
            await storage.saveNodes(nodes);
          },
        );
      ...
      case AppPage.settings:
        return _SettingsPage(
          ...
          onChanged: () async {
            await storage.saveSettings(settings);
            await WindowsIntegration.setStartWithWindows(
...
String _pageSubtitle(AppPage page) {
  switch (page) {
    ...
    case AppPage.subscriptions:
      return '管理订阅来源并同步节点';
    case AppPage.serviceStatus:
      return '查看核心、IPC 与实时运行状态';
    case AppPage.connection:
      return '配置 TUN、DNS 与路由行为';
    case AppPage.systemProxy:
      return '控制 Windows 系统代理设置';
    case AppPage.networkTools:
      return '批量测试节点延迟与连通性';
    case AppPage.config:
      return '智能分流、规则组、自定义规则与配置导入';
    case AppPage.connections:
      return '查看活动与最近关闭的网络连接';
    case AppPage.logs:
      return '查看服务运行日志与错误信息';
    case AppPage.settings:
      return '常规、启动、端口与高级参数';
    case AppPage.about:
      return '版本信息与运行环境';
  }
}
...
class _WindowButtonState extends State<_WindowButton> {
  ...
  Widget build(BuildContext context) {
    final background = hovered
        ...
        : Colors.transparent;
    final foreground = hovered && widget.close
        ? Colors.white
      ...
      child: MouseRegion(
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            child: Icon(
              widget.icon,
              size: widget.icon == Icons.crop_square_rounded ? 15 : 18,
              color: foreground,
            ),
          ),
        ),
...
class _Sidebar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final width = collapsed ? 72.0 : (compact ? 196.0 : 236.0);
    return AnimatedContainer(
      ...
      decoration: const BoxDecoration(
        ...
        border: Border(right: BorderSide(color: Color(0xFFE5EBF3))),
      ),
              ...
              if (!collapsed)
                Padding(
                  ...
                  child: Row(
                    children: <Widget>[
                      Text(
                        coreAvailable ? '核心：HongdaCore' : '核心：未安装',
                        style: TextStyle(
                          color: coreAvailable
                              ? const Color(0xFF667085)
                              : const Color(0xFFC2413B),
                          fontSize: 9.5,
                          fontWeight: coreAvailable
                              ? FontWeight.w500
                              : FontWeight.w700,
                        ),
                      ),
                      const Spacer(),
                      Text(
                        controller.ipcConnected
                            ? 'IPC 正常'
                            : (coreAvailable ? 'IPC 待机' : 'Core 缺失'),
                        style: TextStyle(
          ...
          child: InkWell(
            ...
            child: AnimatedContainer(
              ...
              padding: EdgeInsets.symmetric(
                horizontal: widget.collapsed ? 8 : 13,
              ),
              decoration: BoxDecoration(
                color: selected
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
            ...
            children: <Widget>[
              ...
              Text(
                connected
                    ? '已连接'
                    : (controller.status == CoreStatus.starting
                          ? startupStageLabel(controller.startupStage)
                          : _shortStatus(controller.status)),
                style: TextStyle(
                  color: connected
                      ? const Color(0xFF12A56A)
                      : const Color(0xFF667085),
          ...
          if (controller.status == CoreStatus.error &&
              controller.lastError != null) ...<Widget>[
...
class _ServiceFailureDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final showDetail =
        detail.trim().isNotEmpty && detail.trim() != message.trim();
    return SafeArea(
      child: Center(
        child: Material(
          color: Colors.transparent,
  ...
  List<NodeProfile> get visibleNodes {
    ...
    var result = widget.nodes.where((node) {
      ...
      return node.name.toLowerCase().contains(query) ||
          ...
          nodeRegion(node).name.toLowerCase().contains(query);
    }).toList();
    if (filter == 'auto') {
      result.sort((a, b) {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final ultraDense = constraints.maxHeight < 680;
        final wide = width >= (ultraDense ? 680 : (dense ? 760 : 940));
        final medium = width >= 640;
        final gap = ultraDense ? 4.0 : (dense ? 6.0 : 10.0);
        ...
        final nodeCard = _DashboardNodeCard(
          ...
          maxRows: ultraDense ? 1 : (dense ? 3 : 5),
          dense: dense,
        );
        ...
        return ClipRect(
          child: SingleChildScrollView(
            ...
            padding: EdgeInsets.only(bottom: ultraDense ? 2 : 8),
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: <Widget>[
                _ConnectionHeroCard(
...
class _ConnectionHeroCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      child: LayoutBuilder(
        builder: (context, constraints) {
          final roomy = constraints.maxWidth >= (ultraDense ? 620 : 720);
          final status = Row(
            children: <Widget>[
              _PowerOrb(
                status: controller.status,
              ...
              SizedBox(width: ultraDense ? 12 : 18),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
          ...
          final metrics = <Widget>[
            ...
            _HeroMetric(
              label: '下载',
              value: _formatSpeed(controller.downloadBytesPerSecond),
            ...
            _HeroMetric(
              label: '上传',
              value: _formatSpeed(controller.uploadBytesPerSecond),
              data: uploadHistory,
              green: true,
              dense: dense,
              ultraDense: ultraDense,
            ...
            _HeroMetric(
              label: '流量',
              value: _formatBytes(controller.totalTrafficBytes),
              data: downloadHistory,
              purple: true,
              dense: dense,
              ultraDense: ultraDense,
              ...
              ? Row(
                  children: <Widget>[
                    SizedBox(
                      width: ultraDense
                          ? 230
                          : (constraints.maxWidth < 900 ? 285 : 360),
                      child: status,
                    ),
                  ...
                  children: <Widget>[
                    ...
                    Row(
                      children: <Widget>[
                        ...
                        _InlineStat(
                          label: '总计使用',
                          value: _formatBytes(controller.lifetimeTrafficBytes),
                        ),
                      ],
                    ),
                  ],
                  ...
                  Align(
                    ...
                    child: Wrap(
                      ...
                      children: <Widget>[
                        _InlineStat(
                          label: '路由模式',
                          value: _routeModeText(settings.routeMode),
                        ),
                        _InlineStat(
                          label: 'DNS',
                          value: settings.dnsMode == 'doh' ? 'DoH' : '自动',
                        ),
                        _InlineStat(
                          label: 'TUN',
                          value: settings.tunEnabled ? '已启用' : '未启用',
                        ),
                        _InlineStat(
                          label: '活动连接',
                          value: '${controller.activeConnections}',
                        ),
                        _InlineStat(
                          label: '连接时间',
                          value: _formatDuration(duration),
...
class _StartupProgress extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final progress = startupStageProgress(
      controller.startupStage,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Row(
            children: <Widget>[
              _StartupCheckpoint(label: 'Service', active: percent >= 13),
              _StartupCheckpoint(label: '配置', active: percent >= 52),
              _StartupCheckpoint(label: 'Core', active: percent >= 73),
              _StartupCheckpoint(label: 'API', active: percent >= 83),
              _StartupCheckpoint(label: 'IPC', active: percent >= 91),
            ],
          ),
        ],
      ),
...
class _PowerOrbState extends State<_PowerOrb>
  ...
  Widget build(BuildContext context) {
    ...
    final busy =
        widget.status == CoreStatus.starting ||
        widget.status == CoreStatus.stopping;
    return MouseRegion(
        ...
        child: AnimatedScale(
          ...
          child: AnimatedBuilder(
            ...
            builder: (context, child) {
              ...
              return Container(
                width: widget.compact ? 64 : 82,
                height: widget.compact ? 64 : 82,
                padding: EdgeInsets.all(widget.compact ? 7 : 9),
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  color: const Color(0xFFF7FAFF),
                  border: Border.all(
                  ...
                  boxShadow: <BoxShadow>[
                    BoxShadow(
                      color: const Color(0xFF1677FF).withValues(alpha: glow),
                      blurRadius: 22,
                      spreadRadius: 2,
                    ),
                  ],
                ...
                child: Container(
                  ...
                  child: busy
                      ? Padding(
                          padding: EdgeInsets.all(widget.compact ? 14 : 19),
...
class _HeroMetric extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final color = green
        ...
        : const Color(0xFF1677FF);
    return Container(
      height: ultraDense ? 56 : (dense ? 72 : 92),
      padding: EdgeInsets.fromLTRB(
        ultraDense ? 8 : 12,
        ultraDense ? 5 : (dense ? 7 : 10),
        ultraDense ? 8 : 12,
        ultraDense ? 4 : (dense ? 5 : 7),
      ),
      decoration: BoxDecoration(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          const Spacer(),
          SizedBox(
            height: ultraDense ? 8 : (dense ? 14 : 22),
            width: double.infinity,
            child: CustomPaint(
...
class _SparkPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      final y =
          size.height -
          (values[i] / maxValue).clamp(0.0, 1.0) * (size.height - 3) -
          1.5;
      if (i == 0)
        path.moveTo(x, y);
      else
...
class _DashboardNodeCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final shown = nodes.take(maxRows).toList();
    return Card(
      child: Padding(
        padding: EdgeInsets.fromLTRB(14, dense ? 8 : 12, 14, dense ? 7 : 10),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            Row(
              children: <Widget>[
                ...
                TextButton(
                  onPressed: onOpenAll,
                  child: const Text('管理全部', style: TextStyle(fontSize: 10.5)),
                ),
              ],
            ),
            ...
            if (shown.isEmpty)
              SizedBox(
                height: dense ? 170 : 238,
                child: const Center(
                  child: _EmptyState(
                    icon: Icons.dns_outlined,
                    title: '暂无节点',
                    subtitle: '添加订阅或导入节点后会显示在这里。',
                  ),
                ),
              )
            else
              for (final node in shown)
                _DashboardNodeRow(
                  ...
                  connected: isRunning && node.id == selectedNode?.id,
            ...
            Text(
              '共 $totalCount 个节点，当前显示 ${shown.length} 个',
    ...
    child: InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(7),
          border: Border.all(
            color: selected ? const Color(0xFF94BFFF) : const Color(0xFFE5EAF1),
          ),
        ),
        child: Text(
          text,
          style: TextStyle(
            fontSize: 10.5,
            fontWeight: FontWeight.w700,
            color: selected ? const Color(0xFF1677FF) : const Color(0xFF344054),
          ),
        ),
      ),
    ),
              ...
              SizedBox(
                ...
                child: AnimatedSwitcher(
                  ...
                  child: widget.testing
                      ...
                      : Tooltip(
                          key: ValueKey<String>(health.label),
                  ...
                  children: <Widget>[
                    ...
                    PopupMenuButton<String>(
                      ...
                      onSelected: (value) {
                        if (value == 'copy') {
                          Clipboard.setData(
                            ClipboardData(text: '${node.server}:${node.port}'),
                          );
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('节点地址已复制')),
                          );
          ...
          children: <Widget>[
            const Row(
              children: <Widget>[
                ...
                Spacer(),
                _LegendDot(color: Color(0xFF1677FF), text: '下载 (MB/s)'),
                SizedBox(width: 10),
                _LegendDot(color: Color(0xFF16B66B), text: '上传 (MB/s)'),
                SizedBox(width: 2),
                Icon(
                  Icons.more_horiz_rounded,
                  size: 17,
            ...
            const SizedBox(height: 4),
            Text(
              '↓ ${_formatSpeed(download.isEmpty ? 0 : download.last)}    '
              '↑ ${_formatSpeed(upload.isEmpty ? 0 : upload.last)}',
              style: const TextStyle(
                fontSize: 9.5,
                color: Color(0xFF667085),
                fontWeight: FontWeight.w600,
            ...
            SizedBox(
              height: ultraDense ? 58 : (dense ? 102 : 145),
              width: double.infinity,
              child: CustomPaint(
                painter: _TrafficPainter(upload: upload, download: download),
              ),
            ),
            const SizedBox(height: 3),
            Row(
...
class _TrafficPainter extends CustomPainter {
  ...
  void _line(
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      final y =
          size.height -
          (data[i] / math.max(maxValue, 12)).clamp(0.0, 1.0) *
              (size.height - 16) -
          8;
    ...
    canvas.drawPath(
      ...
      Paint()
        ..shader = ui.Gradient.linear(
          ...
          <Color>[color.withValues(alpha: .18), color.withValues(alpha: .01)],
        ),
    );
    canvas.drawPath(
      path,
...
class _LegendDot extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(
    ...
    children: <Widget>[
      ...
      Text(
        text,
        style: const TextStyle(fontSize: 8.5, color: Color(0xFF667085)),
      ),
    ],
  );
}
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          SizedBox(height: ultraDense ? 3 : (dense ? 6 : 11)),
          LayoutBuilder(
            builder: (context, constraints) {
              final narrow = !ultraDense && constraints.maxWidth < 330;
              ...
              return GridView.count(
                crossAxisCount: ultraDense ? 4 : 2,
                childAspectRatio: ultraDense ? 1.75 : (dense ? 4.2 : 3.1),
  ...
  Widget build(BuildContext context) => MouseRegion(
    ...
    child: InkWell(
      ...
      child: AnimatedContainer(
        ...
        padding: EdgeInsets.symmetric(
          horizontal: widget.ultraDense ? 3 : 10,
          vertical: widget.ultraDense ? 3 : (widget.dense ? 4 : 8),
        ),
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(9),
          border: Border.all(
            color: hovered ? const Color(0xFFCFDDF0) : const Color(0xFFE6EBF2),
          ),
        ),
        child: Row(
          children: <Widget>[
            Icon(
              widget.icon,
              size: widget.ultraDense ? 15 : 18,
              color: widget.color,
            ),
            SizedBox(width: widget.ultraDense ? 3 : 9),
            Expanded(
              child: Text(
                widget.text,
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: TextStyle(
                  fontSize: widget.ultraDense ? 8.5 : 10.5,
...
class _DashboardSubscriptionsCard extends StatelessWidget {
  const _DashboardSubscriptionsCard({
    required this.subscriptions,
    required this.nodes,
  ...
  @override
  Widget build(BuildContext context) {
    final items = subscriptions.take(dense ? 2 : 3).toList();
    return Card(
      child: Padding(
        padding: EdgeInsets.fromLTRB(14, dense ? 7 : 11, 14, dense ? 7 : 11),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            Row(
              children: <Widget>[
                ...
                TextButton(
                  onPressed: onOpen,
                  child: const Text('管理订阅', style: TextStyle(fontSize: 10)),
                ),
              ],
            ),
            ...
            if (items.isEmpty)
              SizedBox(
                ...
                child: const Center(
                  child: Text(
                    '暂无订阅',
                    style: TextStyle(fontSize: 10.5, color: Color(0xFF98A2B3)),
                  ),
                ),
              )
            else
...
class _DashboardConnectionSettingsCard extends StatelessWidget {
  const _DashboardConnectionSettingsCard({
    required this.settings,
    required this.onOpen,
    this.dense = false,
  });
  ...
  Widget build(BuildContext context) => Card(
    child: Padding(
      padding: EdgeInsets.fromLTRB(14, dense ? 7 : 11, 14, dense ? 6 : 10),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: <Widget>[
          Row(
          ...
          Row(
            children: <Widget>[
              ...
              const Spacer(),
              Text(
                _routeModeText(settings.routeMode),
                style: const TextStyle(
                  fontSize: 9.5,
                  color: Color(0xFF1677FF),
                  fontWeight: FontWeight.w700,
...
class _DashboardSystemCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final duration = controller.connectedAt == null
        ? Duration.zero
        : DateTime.now().difference(controller.connectedAt!);
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            _SystemLine(
              ...
              label: '服务模式',
              value: settings.tunEnabled
                  ? (settings.systemProxyEnabled ? 'TUN + 系统代理' : 'TUN')
                  : (settings.systemProxyEnabled ? '系统代理' : '核心代理'),
              green: controller.isRunning,
              dense: dense,
            ),
            if (settings.tunEnabled)
              Tooltip(
                message: controller.tunRoutingDetail,
                child: _SystemLine(
                  icon: Icons.route_rounded,
                  label: 'TUN 路由',
                  value: controller.tunRoutingVerified == true
                      ? '已验证'
                      : (controller.tunRoutingVerified == false
                            ? '未就绪'
                            : '未运行'),
                  green: controller.tunRoutingVerified == true,
                  dense: dense,
                ),
              ),
            _SystemLine(
...
class _SystemLine extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Padding(
    padding: EdgeInsets.symmetric(vertical: dense ? 2.5 : 5),
    child: Row(
      children: <Widget>[
        ...
        Text(
          label,
          style: const TextStyle(fontSize: 9.4, color: Color(0xFF667085)),
        ),
        const Spacer(),
        if (green)
          Container(
        ...
        Flexible(
          child: Text(
            ...
            style: TextStyle(
              fontSize: 9.4,
              color: green ? const Color(0xFF11A86B) : const Color(0xFF344054),
              fontWeight: FontWeight.w600,
            ),
          ),
        ),
...
class _NodesPage extends StatefulWidget {
  const _NodesPage({
    ...
    required this.onSelect,
    required this.onChanged,
    required this.locatingRegions,
    required this.onDetectRegions,
  });
  ...
  final bool locatingRegions;
  final Future<void> Function() onDetectRegions;
...
class _NodesPageState extends State<_NodesPage> {
              ...
              final filters = Wrap(
                ...
                children: <Widget>[
                  ...
                  _FilterButton(
                    text: '收藏',
                    selected: filter == 'favorite',
                    onTap: () => setState(() => filter = 'favorite'),
                  ),
                  _FilterButton(
                    text: '自动选择',
                    selected: filter == 'auto',
                    onTap: () => setState(() => filter = 'auto'),
                  ),
                ],
              );
              final searchBox = SizedBox(
                ...
                child: TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    hintText: '搜索节点名称、国家/地区、地址或协议',
                  ),
                ),
              );
              final actionButtons = <Widget>[
                OutlinedButton.icon(
                  onPressed: widget.locatingRegions
                      ? null
                      : widget.onDetectRegions,
                  icon: widget.locatingRegions
                      ? const SizedBox(
                          width: 14,
                          height: 14,
                  ...
                  label: Text(widget.locatingRegions ? '定位中' : '检测地区'),
                ),
                OutlinedButton.icon(
                  ...
                  child: filtered.isEmpty
                      ? const Center(
                          child: _EmptyState(
                            icon: Icons.dns_outlined,
                            title: '没有匹配的节点',
                            subtitle: '可以添加节点、更新订阅，或清除当前筛选条件。',
                          ),
                        )
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return _NodeManagementRow(
                              ...
                              connected:
                                  ...
                                  node.id == widget.selectedNode?.id,
                              ...
                              onSetRegion: () => _setNodeRegion(node),
  ...
  Future<void> _setNodeRegion(NodeProfile node) async {
    final selected = await showDialog<String?>(
      context: context,
      builder: (context) {
        var value = node.regionCode ?? nodeRegion(node).code;
        if (value == 'ZZ') value = '';
        return StatefulBuilder(
          builder: (context, setDialogState) => AlertDialog(
            title: Text('设置“${node.name}”的国家/地区'),
            content: SizedBox(
              width: 360,
              child: DropdownButtonFormField<String>(
                ...
                items: <DropdownMenuItem<String>>[
                  ...
                  for (final region in supportedNodeRegions)
    ...
    final selectedName = selected.isEmpty
        ? null
        : supportedNodeRegions
              .firstWhere((region) => region.code == selected)
              .name;
    setState(() {
      node.regionCode = selected.isEmpty ? null : selected;
      node.regionName = selectedName;
      node.regionSource = selected.isEmpty ? null : 'manual';
      node.regionLookupAt = null;
  ...
  Future<void> _showImportDialog() async {
    ...
    if (imported.isEmpty) {
      if (mounted)
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('没有识别到支持的节点链接')));
      return;
    }
    final fingerprints = widget.nodes.map((node) => node.fingerprint).toSet();
    final added = imported
        .where((node) => fingerprints.add(node.fingerprint))
        .toList();
    ...
    if (mounted)
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '已导入 ${added.length} 个节点${added.length != imported.length ? '，已自动跳过重复节点' : ''}',
          ),
        ),
      );
  }
...
class _NodeManagementRowState extends State<_NodeManagementRow> {
        ...
        return MouseRegion(
          ...
          child: InkWell(
            ...
            child: AnimatedContainer(
              ...
              child: compact
                  ? _compactRow(node, health)
                  : _desktopRow(node, health),
            ),
          ),
        );
      },
  ...
  Widget _compactRow(NodeProfile node, _NodeHealth health) {
    return Row(
      children: <Widget>[
        Container(
          width: 38,
          ...
          child: RegionBadge(region: nodeRegion(node), width: 28, height: 20),
        ),
        const SizedBox(width: 10),
        Expanded(
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                '${nodeRegion(node).name} · ${node.protocol.toUpperCase()} · ${node.server}:${node.port}',
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: const TextStyle(fontSize: 9.1, color: Color(0xFF667085)),
              ),
            ],
          ),
        ),
              ...
              : Container(
                  ...
                  decoration: BoxDecoration(
                    color: node.latencyMs == null
                        ...
                        : _latencyColor(node.latencyMs!).withValues(alpha: .09),
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Text(
        ...
        PopupMenuButton<String>(
          ...
          itemBuilder: (context) => <PopupMenuEntry<String>>[
            const PopupMenuItem<String>(value: 'test', child: Text('测试延迟')),
            ...
            const PopupMenuItem<String>(value: 'delete', child: Text('删除节点')),
          ],
        ),
      ],
    );
  ...
  Widget _desktopRow(NodeProfile node, _NodeHealth health) {
    return Row(
      children: <Widget>[
        Expanded(
          flex: 5,
        ...
        SizedBox(
          ...
          child: Row(
            children: <Widget>[
              ...
              Text(
                health.label,
                style: TextStyle(fontSize: 9, color: health.color),
              ),
            ],
          ),
        ),
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              PopupMenuButton<String>(
                tooltip: '更多操作',
                onSelected: (value) {
                  if (value == 'region') widget.onSetRegion();
                ...
                itemBuilder: (context) => const <PopupMenuEntry<String>>[
                  ...
                  PopupMenuDivider(),
                  PopupMenuItem<String>(value: 'delete', child: Text('删除节点')),
                ],
...
class _SubscriptionsPageState extends State<_SubscriptionsPage> {
            ...
            builder: (context, constraints) {
              ...
              final actions = Wrap(
                ...
                children: <Widget>[
                  OutlinedButton.icon(
                    onPressed:
                        updating.isEmpty && widget.subscriptions.isNotEmpty
                        ? _updateAll
                        : null,
              ...
              if (constraints.maxWidth < 760) {
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[info, const SizedBox(height: 9), actions],
                );
              }
              return Row(
                children: <Widget>[
            ...
            child: widget.subscriptions.isEmpty
                ? const Center(
                    child: _EmptyState(
                      ...
                      title: '暂无订阅',
                      subtitle: '添加订阅 URL 后，可自动解析并同步节点。',
                    ),
                  )
                : ListView.builder(
  ...
  Future<void> _updateAll() async {
    ...
    if (mounted)
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('全部订阅已更新完成')));
  }
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: <Widget>[
                const Text(
                  '支持 Clash / Mihomo / sing-box / Base64 节点订阅。',
                ...
                CheckboxListTile(
                  ...
                  onChanged: (value) =>
                      setDialogState(() => allowInsecureHttp = value == true),
                  title: const Text('允许不安全的 HTTP 订阅'),
                  subtitle: const Text('仅用于可信局域网地址；HTTPS 订阅不会被允许降级到 HTTP。'),
                ),
              ],
            ),
          ),
  ...
  Future<void> _updateSubscription(
    ...
    try {
      ...
      final previous = widget.nodes
          .where((node) => node.source == sub.id)
          .toList(growable: false);
      final byFingerprint = <String, NodeProfile>{
        for (final node in previous) node.fingerprint: node,
      };
      final byIdentity = <String, NodeProfile>{
        for (final node in previous) node.identityKey: node,
      };
      final endpointCounts = <String, int>{};
      final byUniqueEndpoint = <String, NodeProfile>{};
      for (final node in previous) {
        endpointCounts[node.endpointKey] =
            (endpointCounts[node.endpointKey] ?? 0) + 1;
        byUniqueEndpoint[node.endpointKey] = node;
      }
      for (final node in imported.nodes) {
        final old =
            byFingerprint[node.fingerprint] ??
            byIdentity[node.identityKey] ??
            (endpointCounts[node.endpointKey] == 1
                ? byUniqueEndpoint[node.endpointKey]
                : null);
        ...
        node.proxyConnectDelayMs = old.proxyConnectDelayMs;
        node.proxyTotalDelayMs = old.proxyTotalDelayMs;
        node.lastLatencyTest = old.lastLatencyTest;
        node.testError = old.testError;
        node.probeStatus = old.probeStatus;
        node.regionCode = old.regionCode;
        node.regionName = old.regionName;
        node.regionSource = old.regionSource;
        node.regionLookupAt = old.regionLookupAt;
        node.regionLookupError = old.regionLookupError;
      }
      widget.nodes.removeWhere((node) => node.source == sub.id);
      widget.groups.removeWhere((group) => group.source == sub.id);
      widget.rules.removeWhere((rule) => rule.source == sub.id);
      ...
      if (mounted && showToast) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '已更新 ${sub.name}：${imported.nodes.length} 节点 · ${imported.groups.length} 规则组 · ${imported.rules.length} 路由规则',
            ),
          ),
        );
      }
    ...
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(10),
          border: Border.all(
            color: hovered ? const Color(0xFFCFE0F6) : const Color(0xFFE5EAF1),
          ),
        ),
        child: Row(
          children: <Widget>[
                  ...
                  Wrap(
                    ...
                    children: <Widget>[
                      Text(
                        sub.lastUpdated == null
                            ? '尚未更新'
                            : '最后更新 ${_dateTime(sub.lastUpdated!)}',
                        style: const TextStyle(
                          fontSize: 9.2,
                          color: Color(0xFF98A2B3),
                        ),
                      ...
                      if (sub.totalBytes != null)
                        Text(
                          '额度 ${_formatBytes((sub.uploadBytes ?? 0) + (sub.downloadBytes ?? 0))} / ${_formatBytes(sub.totalBytes!)}',
                          style: const TextStyle(
                            fontSize: 9.2,
                            color: Color(0xFF7B8798),
                          ),
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      _SystemLine(
                        ...
                        label: 'IPC 状态',
                        value: controller.ipcConnected ? '已连接' : '未连接',
                        green: controller.ipcConnected,
                      ),
                      _SystemLine(
                        icon: Icons.memory_rounded,
                        label: '核心状态',
                        value: _shortStatus(controller.status),
                        green: controller.isRunning,
                      ),
                      _SystemLine(
                        icon: Icons.input_rounded,
        ...
        child: Column(
          ...
          children: <Widget>[
                ...
                const Expanded(
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      SizedBox(height: 3),
                      Text(
                        '智能分流采用 Karing 式路由：自定义规则优先，国内与下载流量直连，OpenAI / GFW 及未命中的国外流量走节点。',
                        style: TextStyle(
                          fontSize: 9.6,
                          color: Color(0xFF667085),
                          height: 1.4,
            ...
            if (settings.routeMode == 'rule') ...<Widget>[
              ...
              _SettingSwitch(
                title: '国内域名 / IP 直连',
                subtitle: '启用 LAN、ChinaDomain、ChinaIp / ChinaIpV6 规则集',
              ...
              _SettingSwitch(
                title: '常用海外站点代理',
                subtitle: '内置 OpenAI 域名并启用 ProxyLite、ProxyGFWlist',
      ...
      children: <Widget>[
        ...
        _SettingsCard(
          ...
          children: <Widget>[
            _SettingSwitch(
              title: 'TUN 模式',
              subtitle: '接管 Windows 系统流量；开启后需要管理员权限',
              value: settings.tunEnabled,
              enabled: !isRunning,
              onChanged: (v) {
                settings.tunEnabled = v;
                if (v) {
                  settings.systemProxyEnabled = false;
                  settings.strictRoute = true;
                  settings.ipv6Enabled = false;
                  settings.dnsStrategy = 'ipv4_only';
                } else if (!settings.tailscaleEnabled) {
                  // TUN 关闭后恢复默认的系统代理模式，避免设置页停留在“关闭”状态。
                  settings.systemProxyEnabled = true;
                }
                onChanged();
              },
              ...
              Padding(
                ...
                child: Align(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: onRestartAdmin,
                    icon: const Icon(Icons.admin_panel_settings_outlined),
                    label: const Text('以管理员身份重启'),
                  ),
                ),
              ),
            _SettingSwitch(
              title: '严格路由',
              subtitle: 'Windows 下可降低多宿主 DNS 泄漏风险；个别虚拟机软件可能受影响',
              value: settings.strictRoute,
              enabled: !isRunning && settings.tunEnabled,
              onChanged: (v) {
                settings.strictRoute = v;
                onChanged();
              },
            ),
            _SettingSwitch(
              title: '断线自动恢复',
              subtitle: 'Service 或 Core 意外退出后按 2 / 4 / 8 秒退避重连，最多尝试 3 次',
              value: settings.autoReconnect,
              enabled: true,
              onChanged: (v) {
                settings.autoReconnect = v;
                onChanged();
              },
            ),
            _SettingSwitch(
              title: '局域网直连',
              subtitle: '私有 IP 地址不经过代理节点',
              value: settings.bypassLan,
              enabled: !isRunning,
              onChanged: (v) {
                settings.bypassLan = v;
                onChanged();
              },
            ),
            _SettingSwitch(
              title: '兼容过期 / 自签名节点证书',
              subtitle: '仅跳过代理节点入口证书校验，目标网站 HTTPS 仍正常校验；只在服务端证书失效时开启',
              value: settings.allowInsecureTls,
              enabled: !isRunning,
              onChanged: (v) {
                settings.allowInsecureTls = v;
                onChanged();
              },
            ),
          ],
        ...
        _SettingsCard(
          ...
          children: <Widget>[
            _SettingSwitch(
              title: 'Tailscale Endpoint 尚未启用',
              subtitle: 'HongdaCore 1.10 暂未实现；旧设置会自动关闭，不会伪装成已生效',
              value: false,
              enabled: false,
              onChanged: (_) {},
            ),
        ...
        _SettingsCard(
          ...
          children: <Widget>[
            const Padding(
              ...
              child: Text(
                '以下为首选端口；连接时如被其他程序占用，会自动选择相邻可用端口并同步到系统代理与控制 API。',
      ...
      children: <Widget>[
        _SettingsCard(
          ...
          children: <Widget>[
            _SettingSwitch(
              title: '连接后自动设置系统代理',
              subtitle: settings.tunEnabled
                  ? 'TUN 模式下关闭，避免 ChatGPT 等 AppContainer 应用被本机回环代理阻断'
                  : '将 Windows 当前用户代理指向 127.0.0.1:${settings.mixedPort}',
              value: settings.systemProxyEnabled,
              enabled: !settings.tunEnabled && !isRunning,
              onChanged: (v) {
                settings.systemProxyEnabled = v;
                onChanged();
              },
            ),
            const Padding(
              ...
              child: Text(
                '关闭连接时会主动清理本软件设置的代理状态，避免出现“代理残留导致无法联网”。',
            ...
            child: Row(
              children: <Widget>[
                Icon(
                  ...
                  color: isRunning
                      ? const Color(0xFF20A866)
                      : const Color(0xFF7C8797),
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: Text(
                    isRunning ? '核心正在运行，系统代理设置可以即时切换。' : '当前未连接；设置将在下次连接时生效。',
                  ),
                ),
              ],
            ),
...
class _NetworkToolsPageState extends State<_NetworkToolsPage> {
  ...
  @override
  Widget build(BuildContext context) {
    final latencyCard = Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: <Widget>[
            const _SectionHeader(title: '节点延迟 / 连通性'),
            const SizedBox(height: 8),
            Text(
              widget.controller.isRunning
                  ? '列表显示节点服务器 TCP RTT；HongdaCore 同时执行真实代理 URLTest，完整链路耗时保留在节点状态与日志中。'
                  : '当前未连接：显示节点服务器 TCP RTT；连接后还会验证完整代理链路。',
              style: const TextStyle(color: Color(0xFF667085), fontSize: 11.5),
            ),
            const SizedBox(height: 12),
            FilledButton.icon(
              onPressed: testing ? null : _testAll,
              icon: testing
                  ? const SizedBox(
                      width: 15,
                      height: 15,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 12),
            OutlinedButton.icon(
              onPressed: _checkPublicIp,
              icon: const Icon(Icons.public_rounded),
              label: const Text('检测当前出口'),
            ),
          ],
        ),
      ),
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      title: Text(n.name),
                      subtitle: Text(
                        '${nodeRegion(n).name} · ${n.server}:${n.port}',
                      ),
                      trailing: Text(
                        n.latencyMs == null ? '--' : '${n.latencyMs} ms',
                        style: TextStyle(
                          fontWeight: FontWeight.w700,
                          color: n.latencyMs == null
                              ? const Color(0xFF98A2B3)
  ...
  Future<void> _testAll() async {
    ...
    for (final node in widget.nodes.where((item) => item.enabled)) {
      await widget.controller.testNode(node, widget.settings);
      if (mounted) setState(() {});
    }
...
class _ConfigPageState extends State<_ConfigPage> {
          ...
          children: <Widget>[
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                final info = Column(
                  ...
                  children: <Widget>[
                    const Text(
                      'Clash / Mihomo · sing-box · Base64 / 分享链接',
                      style: TextStyle(
                        fontWeight: FontWeight.w800,
                        color: Color(0xFF1D2939),
                      ),
                ...
                final buttons = Wrap(
                  ...
                  children: <Widget>[
                    OutlinedButton.icon(
                      onPressed: _loadSelectedOutbound,
                      icon: const Icon(Icons.code_rounded, size: 16),
                      label: const Text('当前节点 JSON'),
                    ),
                    OutlinedButton.icon(
                    ...
                    FilledButton.icon(
                      onPressed: preview == null ? null : _applyImport,
            ...
            Expanded(
              child: TextField(
                controller: editor,
                onChanged: (_) {
                  if (preview != null || parseError != null)
                    setState(() {
                      preview = null;
                      parseError = null;
                    });
        ...
        _ToolbarCard(
          child: Row(
            children: <Widget>[
              Expanded(
                child: Text(
                  '${widget.groups.length} 个规则组 · Selector / URLTest 可直接写入 HongdaCore 运行配置',
                  style: const TextStyle(
                    fontSize: 10.8,
                    color: Color(0xFF667085),
                  ),
            ...
            child: widget.groups.isEmpty
                ? const Center(
                    child: _EmptyState(
                      ...
                      title: '暂无规则组',
                      subtitle: '可以创建手动选择组或 URLTest 自动测速组。',
                    ),
                  )
                : ListView.separated(
                    padding: const EdgeInsets.all(10),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _ConfigListTile(
                        icon: group.type == 'urltest'
                            ? Icons.speed_rounded
                            : Icons.hub_outlined,
                        title: group.name,
                        subtitle:
                            '${group.type == 'urltest' ? 'URLTest' : 'Selector'} · ${group.nodeIds.length} 节点${group.source == 'manual' ? '' : ' · ${group.source}'}',
                        enabled: group.enabled,
                        onToggle: (v) {
                          setState(() => group.enabled = v);
                          widget.onChanged();
                        },
        ...
        _ToolbarCard(
          child: Row(
            children: <Widget>[
              Expanded(
                child: Text(
                  '${widget.rules.length} 条路由规则 · 支持域名、后缀、关键词、CIDR 和 TCP/UDP；进程匹配开发中',
                  style: const TextStyle(
                    fontSize: 10.8,
                    color: Color(0xFF667085),
                  ),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Padding(
                        ...
                        child: _ConfigListTile(
                          ...
                          title: rule.name,
                          subtitle:
                              '${_ruleSummary(rule)} → ${_outboundLabel(rule.outbound)}',
  ...
  void _loadSelectedOutbound() {
    ...
    if (node == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('当前没有选中节点')));
      return;
    }
    editor.text = prettyJson(<String, dynamic>{
      'outbounds': <Map<String, dynamic>>[
        <String, dynamic>{...node.outbound, 'tag': node.name},
      ],
    });
    setState(() {
      preview = null;
  ...
  void _applyImport() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          '导入完成：新增 $addedNodes 节点 · ${value.groups.length} 规则组 · ${value.rules.length} 路由规则',
        ),
      ),
    );
  ...
  Future<void> _editGroup([ProxyGroupProfile? current]) async {
    ...
    final url = TextEditingController(
      text: current?.testUrl ?? widget.settings.urlTestUrl,
    );
    final interval = TextEditingController(
      text: (current?.intervalSeconds ?? widget.settings.urlTestIntervalSeconds)
          .toString(),
    );
    final tolerance = TextEditingController(
      text: (current?.toleranceMs ?? widget.settings.urlTestToleranceMs)
          .toString(),
    );
              ...
              child: Column(
                children: <Widget>[
                      ...
                      SizedBox(
                        ...
                        child: DropdownButtonFormField<String>(
                          ...
                          items: const <DropdownMenuItem<String>>[
                            DropdownMenuItem(
                              value: 'selector',
                              child: Text('手动选择 Selector'),
                            ),
                            DropdownMenuItem(
                              value: 'urltest',
                              child: Text('自动测速 URLTest'),
                            ),
                          ],
                          onChanged: (v) {
                  ...
                  Align(
                    alignment: Alignment.centerLeft,
                    child: Text(
                      '成员节点（${selectedIds.length}/${widget.nodes.length}）',
                  ...
                  Expanded(
                    child: Card(
                      ...
                      child: RadioGroup<String>(
                        groupValue: selectedNodeId,
                        onChanged: (value) =>
                            setLocal(() => selectedNodeId = value ?? ''),
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, i) {
                            ...
                            return CheckboxListTile(
                              ...
                              secondary: type == 'selector' && checked
                                  ? Radio<String>(value: node.id)
                                  : null,
                              onChanged: (v) => setLocal(() {
                                if (v == true) {
                                  ...
                                  if (selectedNodeId == node.id) {
                                    selectedNodeId = selectedIds.isEmpty
                                        ? ''
                                        : selectedIds.first;
                                  }
                                }
    ...
    if (ok == true) {
      final group =
          ...
          ProxyGroupProfile(
            ...
            name: name.text.trim().isEmpty ? '规则组' : name.text.trim(),
          );
      group.name = name.text.trim().isEmpty ? '规则组' : name.text.trim();
      group.type = type;
      group.nodeIds = selectedIds.toList();
      group.selectedNodeId = selectedNodeId;
      group.testUrl = url.text.trim().isEmpty
          ? widget.settings.urlTestUrl
          : url.text.trim();
      group.intervalSeconds = (int.tryParse(interval.text) ?? 180)
          .clamp(10, 86400)
          .toInt();
      group.toleranceMs = (int.tryParse(tolerance.text) ?? 50)
          .clamp(0, 10000)
          .toInt();
  ...
  Future<void> _deleteGroup(ProxyGroupProfile group) async {
    final ok = await _confirm(
      '删除规则组',
      '确定删除“${group.name}”？引用该组的路由规则会回退到默认代理。',
    );
    if (!ok) return;
    setState(() => widget.groups.removeWhere((g) => g.id == group.id));
    for (final rule in widget.rules.where(
      (r) => r.outbound == 'group:${group.id}',
    ))
      rule.outbound = 'proxy';
    if (widget.settings.selectedGroupId == group.id)
      widget.settings.selectedGroupId = '';
    widget.onChanged();
  }
  ...
  Future<void> _editRule([RouteRuleProfile? current]) async {
    ...
    final domains = TextEditingController(
      text: current?.domains.join('\n') ?? '',
    );
    final suffixes = TextEditingController(
      text: current?.domainSuffixes.join('\n') ?? '',
    );
    final keywords = TextEditingController(
      text: current?.domainKeywords.join('\n') ?? '',
    );
    final cidrs = TextEditingController(
      text: current?.ipCidrs.join('\n') ?? '',
    );
    final processes = TextEditingController(
      text: current?.processNames.join('\n') ?? '',
    );
    ...
    final validOutbounds = <String>{
      ...
      'direct',
      'block',
      ...widget.groups.map((g) => 'group:${g.id}'),
      ...widget.nodes.map((n) => 'node:${n.id}'),
    };
              ...
              child: Column(
                children: <Widget>[
                    ...
                    children: <Widget>[
                      Expanded(
                        child: DropdownButtonFormField<String>(
                          ...
                          decoration: const InputDecoration(labelText: '网络'),
                          items: const <DropdownMenuItem<String>>[
                          ...
                          items: <DropdownMenuItem<String>>[
                            ...
                            ...widget.nodes
                                ...
                                .map(
                                  (n) => DropdownMenuItem(
                                    value: 'node:${n.id}',
                                    child: Text('节点 · ${n.name}'),
                                  ),
                                ),
                          ],
                          onChanged: (v) =>
                              setLocal(() => outbound = v ?? 'proxy'),
                        ),
                      ),
                    ],
                  ),
                  ...
                  _RuleEditorField(
                    controller: domains,
                    label: '精确域名',
                    hint: 'example.com，每行一个',
                  ),
                  _RuleEditorField(
                    controller: suffixes,
                    label: '域名后缀',
                    hint: 'google.com，每行一个',
                  ),
                  _RuleEditorField(
                    controller: keywords,
                    label: '域名关键词',
                  ...
                  _RuleEditorField(
                    controller: cidrs,
                    label: 'IP / CIDR',
                    hint: '1.1.1.0/24，每行一个',
                  ),
                  _RuleEditorField(
                    controller: processes,
                    label: '进程名（HongdaCore 1.10 暂不支持）',
                    hint: '请留空；旧规则可在这里清除',
                  ),
                ],
              ),
    ...
    final processRules = _lines(processes.text);
    if (ok == true && processRules.isNotEmpty && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('HongdaCore 1.10 暂不支持进程匹配，请清空进程名后保存')),
      );
    }
    if (ok == true && processRules.isEmpty) {
      final rule =
          current ??
          RouteRuleProfile(
            id: 'rule-${DateTime.now().microsecondsSinceEpoch}',
            name: name.text.trim().isEmpty ? '新规则' : name.text.trim(),
          );
      ...
      rule.processNames = const <String>[];
  ...
  String _ruleSummary(RouteRuleProfile r) {
    ...
    if (r.domainKeywords.isNotEmpty)
      parts.add('关键词 ${r.domainKeywords.length}');
    ...
    child: Row(
      children: <Widget>[
        ...
        Container(
          ...
          child: Icon(icon, size: 18, color: const Color(0xFF1677FF)),
        ),
        const SizedBox(width: 10),
        Expanded(
          child: Column(
...
class _LogsPageState extends State<_LogsPage> {
  ...
  void initState() {
    ...
    if (_mode == 'connections') {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        widget.controller.refreshConnectionLog(widget.settings);
      });
    }
  }
  ...
  @override
  Widget build(BuildContext context) {
    final connectionMode = _mode == 'connections';
    final entries = _filteredConnections();
    final active = widget.controller.connectionLogs
    ...
    return Column(
      children: <Widget>[
        Row(
          children: <Widget>[
            ...
            Expanded(
              child: Text(
                connectionMode
                    ? '$active 个活跃连接 · ${widget.controller.connectionLogs.length} 条记录'
                    : '实时日志 · ${widget.controller.logs.length} 行',
            ...
            if (connectionMode) ...<Widget>[
              IconButton(
                ...
                onPressed: widget.controller.isRunning
                    ? () => widget.controller.refreshConnectionLog(
                        widget.settings,
                      )
                    : null,
            ...
            OutlinedButton.icon(
              onPressed: connectionMode
                  ? (entries.isEmpty ? null : () => _copyConnections(entries))
                  : () => Clipboard.setData(
                      ClipboardData(text: widget.controller.logs.join('\n')),
                    ),
              icon: const Icon(Icons.copy_rounded),
              label: Text(connectionMode ? '复制连接' : '复制日志'),
            ),
            if (connectionMode) ...<Widget>[
              const SizedBox(width: 8),
              OutlinedButton.icon(
                onPressed: widget.controller.connectionLogs.isEmpty
                    ? null
                    : () =>
                          widget.controller.clearConnectionLog(widget.settings),
            ...
            OutlinedButton.icon(
              onPressed: () =>
                  WindowsIntegration.openFolder(widget.storage.baseDir.path),
        ...
        if (connectionMode) ...<Widget>[
          ...
          TextField(
            ...
            decoration: const InputDecoration(
              ...
              hintText: '筛选域名、目标 IP、协议、节点或命中规则',
              isDense: true,
            ),
          ),
        ],
        const SizedBox(height: 12),
        Expanded(
          child: connectionMode
              ? _buildConnectionTable(entries)
              : _buildRuntimeLog(),
        ),
      ],
    );
      ...
      child: widget.controller.logs.isEmpty
          ? const Center(
              child: _EmptyState(
                ...
                title: '暂无日志',
                subtitle: '启动连接后，这里显示 HongdaCore 标准输出和错误信息。',
              ),
            )
          : ListView.builder(
              reverse: true,
              ...
              itemBuilder: (context, index) {
                final text = widget
                    .controller
                    .logs[widget.controller.logs.length - 1 - index];
  ...
  Widget _buildConnectionTable(List<ConnectionLogEntry> entries) {
    if (entries.isEmpty) {
      return const Card(
        child: Center(
          child: _EmptyState(
            icon: Icons.account_tree_outlined,
            title: '暂无连接记录',
            subtitle: '连接建立后，这里会显示域名、目标地址、出站节点、规则和流量。',
          ),
        ),
      );
    }
    return Card(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          final width = math.max(1220.0, constraints.maxWidth);
              ...
              child: Column(
                children: <Widget>[
                  Container(
                    ...
                    child: const Row(
                      children: <Widget>[
                        _ConnectionHeader('状态', 70),
                        _ConnectionHeader('开始时间', 92),
                        _ConnectionHeader('协议', 62),
                        _ConnectionHeader('目标域名 / 地址', 330),
                        _ConnectionHeader('出站节点', 160),
                        _ConnectionHeader('命中规则', 220),
                        _ConnectionHeader('上传', 105),
                        _ConnectionHeader('下载', 105),
                        _ConnectionHeader('总量', 105),
                      ],
                    ),
                  ),
                  ...
                  Expanded(
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) =>
                          _buildConnectionRow(entries[index]),
                    ),
                  ),
                ],
              ),
  ...
  Widget _buildConnectionRow(ConnectionLogEntry entry) {
    final target = entry.domain.isEmpty ? entry.destination : entry.domain;
    final detail = entry.domain.isEmpty ? entry.source : entry.destination;
    return Container(
      constraints: const BoxConstraints(minHeight: 48),
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
      color: entry.active ? Colors.white : const Color(0xFFFBFCFE),
      child: Row(
        children: <Widget>[
          SizedBox(
            width: 70,
          ...
          _connectionCell(_connectionTime(entry.startedAt), 92),
          _connectionCell(entry.network, 62, monospace: true),
          SizedBox(
            width: 330,
            child: Tooltip(
              message: <String>[
                if (entry.domain.isNotEmpty) '域名：${entry.domain}',
                '目标：${entry.destination}',
                if (entry.source.isNotEmpty) '来源：${entry.source}',
              ].join('\n'),
              child: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  ...
                  if (detail.isNotEmpty && detail != target)
                    Text(
                      detail,
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
          ...
          _connectionCell(
            widget.controller.connectionOutboundLabel(entry),
            160,
          ),
          _connectionCell(entry.rule.isEmpty ? entry.route : entry.rule, 220),
          _connectionCell(_formatBytes(entry.upload), 105, monospace: true),
          _connectionCell(_formatBytes(entry.download), 105, monospace: true),
          _connectionCell(_formatBytes(entry.total), 105, monospace: true),
        ],
      ),
    );
  }

  Widget _connectionCell(String text, double width, {bool monospace = false}) {
    return SizedBox(
      width: width,
      child: Text(
        text.isEmpty ? '—' : text,
  ...
  List<ConnectionLogEntry> _filteredConnections() {
    final keyword = _search.text.trim().toLowerCase();
    if (keyword.isEmpty) return widget.controller.connectionLogs;
    return widget.controller.connectionLogs.where((entry) {
      final node = widget.controller.connectionOutboundLabel(entry);
      return <String>[
        entry.network,
        entry.source,
        entry.destination,
        ...
        entry.rule,
        node,
        entry.active ? '活动 active' : '关闭 closed',
      ].any((value) => value.toLowerCase().contains(keyword));
    }).toList();
  }

  Future<void> _copyConnections(List<ConnectionLogEntry> entries) {
    final lines = <String>['状态\t开始时间\t协议\t域名\t目标\t出站\t规则\t上传\t下载'];
    for (final entry in entries) {
      lines.add(
        <String>[
          entry.active ? '活动' : '已关闭',
          _connectionTime(entry.startedAt),
          entry.network,
          entry.domain,
          entry.destination,
          widget.controller.connectionOutboundLabel(entry),
  ...
  String _connectionTime(DateTime? value) {
    if (value == null) return '—';
...
class _ConnectionHeader extends StatelessWidget {
  const _ConnectionHeader(this.text, this.width);
  final String text;
  final double width;
      ...
      children: <Widget>[
        _SettingsCard(
          ...
          children: <Widget>[
            _SettingSwitch(
              title: '开机启动',
              subtitle: '登录 Windows 后自动启动鸿达星轨智连',
              value: widget.settings.startWithWindows,
              onChanged: (v) {
                widget.settings.startWithWindows = v;
                widget.onChanged();
              },
            ),
            Padding(
              ...
              child: DropdownButtonFormField<String>(
                initialValue: widget.settings.logLevel,
                decoration: const InputDecoration(labelText: '日志等级'),
                items: const <DropdownMenuItem<String>>[
        ...
        const SizedBox(height: 16),
        _SettingsCard(
          title: 'HongdaService · HongdaCore',
          children: <Widget>[
            ListTile(
              leading: const Icon(
                Icons.memory_rounded,
                color: Color(0xFF0B72E7),
              ),
              title: Text(
                widget.coreVersion ?? '服务未构建/缺失',
                style: const TextStyle(fontWeight: FontWeight.w700),
              ),
              subtitle: Text(
                widget.coreVersion == null
                    ? '未检测到 HongdaService.exe。正常发布包应自带内嵌 HongdaCore 的 Service，请检查 runtime/service 是否完整。'
            ...
            const Padding(
              ...
              child: Text(
                '核心能力：VLESS · Trojan · Hysteria2 · TUIC · Reality · URLTest · 远程规则集 · Clash API · TUN / DoH',
                style: TextStyle(fontSize: 11, color: Color(0xFF667085)),
              ),
            ),
          ],
              ...
              children: <Widget>[
                ...
                Expanded(
                  child: Column(
                    ...
                    children: <Widget>[
                      Text(
                        widget.isAdmin ? '当前为管理员权限' : '当前为标准权限',
                        style: const TextStyle(fontWeight: FontWeight.w700),
                      ),
                      const SizedBox(height: 3),
                      const Text(
                        '只有使用 TUN 时才需要管理员权限；普通系统代理模式不强制 UAC。',
                        style: TextStyle(
                          fontSize: 12,
                          color: Color(0xFF667085),
                        ),
...
class _PortSettingState extends State<_PortSetting> {
  ...
  @override
  void didUpdateWidget(covariant _PortSetting oldWidget) {
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          return Row(
            children: <Widget>[
              _BottomStatusItem(
                icon: connected ? Icons.hub_rounded : Icons.hub_outlined,
                text: '连接类型：${selectedNode?.protocol.toUpperCase() ?? '--'}',
              ),
              if (!compact) ...<Widget>[
                const SizedBox(width: 24),
                _BottomStatusItem(
                  icon: Icons.cloud_outlined,
                  text: '代理模式：${_routeModeText(settings.routeMode)}',
                ),
                const SizedBox(width: 24),
                _BottomStatusItem(
                  icon: Icons.shield_outlined,
                  text: '系统代理：${settings.systemProxyEnabled ? '已启用' : '未启用'}',
                ),
                const SizedBox(width: 24),
                _BottomStatusItem(
                  icon: Icons.security_rounded,
                  text: 'TUN 模式：${settings.tunEnabled ? '已启用' : '未启用'}',
                ),
              ],
              const Spacer(),
              _BottomStatusItem(
                icon: Icons.north_rounded,
                text: _formatSpeed(uploadSpeed),
              ),
              const SizedBox(width: 18),
              _BottomStatusItem(
                icon: Icons.south_rounded,
                text: _formatSpeed(downloadSpeed),
              ),
              if (!compact) ...<Widget>[
                const SizedBox(width: 22),
                _BottomStatusItem(
                  icon: Icons.account_tree_outlined,
                  text: '$activeConnections 个连接',
                ),
                const SizedBox(width: 22),
                _BottomStatusItem(
                  icon: Icons.memory_rounded,
                  text: coreVersion == null ? 'Core 缺失' : 'Core Ready',
                ),
              ],
            ],
          );
...
_NodeHealth _nodeHealth(
  NodeProfile node, {
  required bool testing,
  required bool connected,
}) {
  if (!node.enabled) {
    return const _NodeHealth(
      label: '禁用',
      detail: '该节点已禁用',
  ...
  if (connected) {
    return const _NodeHealth(
      label: '已连接',
      detail: '当前 Core 正在使用该节点',
      icon: Icons.link_rounded,
      color: Color(0xFF0DB46C),
    );
  }
  switch (node.probeStatus) {
    case NodeProbeStatus.proxyAvailable:
      final details = <String>[
        if (node.latencyMs != null) '节点 RTT ${node.latencyMs} ms',
        if (node.proxyConnectDelayMs != null)
          '代理传输建立 ${node.proxyConnectDelayMs} ms',
        if (node.proxyTotalDelayMs != null) '完整链路 ${node.proxyTotalDelayMs} ms',
      ];
      return _NodeHealth(
        label: '代理可用',
        detail: details.isEmpty ? '已通过完整代理链路测试' : details.join(' · '),
        icon: Icons.check_circle_rounded,
        color: const Color(0xFF0DB46C),
      );
    case NodeProbeStatus.tcpReachable:
      return _NodeHealth(
        label: 'TCP可达',
        detail: node.latencyMs == null
            ? '服务器端口可达，尚未验证代理握手'
            : '服务器端口可达：${node.latencyMs} ms；尚未验证代理握手',
        icon: Icons.cable_rounded,
        color: const Color(0xFFF79009),
      );
    case NodeProbeStatus.failed:
      return _NodeHealth(
        label: '失败',
        detail: node.testError?.trim().isNotEmpty == true
            ? node.testError!
            : '最近一次测试失败',
        icon: Icons.cancel_rounded,
        color: const Color(0xFFF04438),
      );
    case NodeProbeStatus.untested:
      return const _NodeHealth(
        label: '未测试',
        detail: '尚未测试该节点',
