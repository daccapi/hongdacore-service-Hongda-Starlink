# 鸿达星轨智连 V1.6.5 — Windows 源码

鸿达星轨智连 Windows 客户端，采用 Flutter 桌面 UI、独立 `HongdaService.exe` 监管进程和本地源码构建的 `HongdaCore.exe`。应用版本为 **1.6.5+165**，Service 版本为 **1.4.2**，核心兼容版本为 **1.13.18**。
...
默认运行包输出为 `dist\Hongda-Starlink-V1.6.5-Windows-x64.zip`。详细步骤见 [BUILD_WINDOWS.md](BUILD_WINDOWS.md)。
