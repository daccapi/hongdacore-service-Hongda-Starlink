# 鸿达星轨智连 V1.6.19 — Windows 源码

鸿达星轨智连 Windows 客户端，采用 Flutter 桌面 UI、独立 `HongdaService.exe` 监管进程和本地源码构建的 `HongdaCore.exe`。应用版本为 **1.6.19+179**，Service 版本为 **1.5.6**，核心版本为 **1.10.6**。

## 运行架构

```text
Hongda Starlink.exe
  └─ HongdaService.exe 1.5.6
       └─ HongdaCore.exe 1.10.6
...
默认运行包输出到项目上级目录：`Hongda-Starlink-V1.6.19-Windows-x64.zip`。详细步骤见 [BUILD_WINDOWS.md](BUILD_WINDOWS.md)。
