...
- DNS 连接池随真实 Selector 节点刷新，切换时取消旧 DNS 请求；缓存按解析器、策略和节点隔离。
- DoH 失败不再偷偷回退系统 DNS；TTL 不再抬高到 30 秒，缓存回复扣除已过去的时间，并保护 EDNS 标志。
- URLTest 与手动测速共用 HEAD/不跟随跳转/仅 2xx 成功规则，全局最多 4 个同时探测，排队时间不计入延迟。
- 自动选节点只接受真实代理探针成功的节点，按完整探针排序；QUIC 节点不再使用 TCP 端口探测。
...
- HongdaCore 新增 Hysteria2 Salamander UDP 混淆，订阅内携带
  `obfs.type=salamander` 的节点不再导致配置校验失败。
- 配置生成器增加逐节点能力检查：未选中的不兼容节点会被跳过并写入日志，不再拖垮
  当前选中的 VLESS/Trojan/Hysteria2/TUIC 连接。
- 用户主动选择不兼容节点时，界面会指出具体原因，例如未知混淆、multiplex、TUIC
  0-RTT 或暂未实现的传输方式。
- Salamander 的数据包变换与配置导入已通过自动测试；因本次未启动 TUN，真实服务器
  互通仍需使用发布包和对应 Hysteria2 节点实测。

详细说明见 [V1.6.24_HYSTERIA2_SALAMANDER_FIX.md](V1.6.24_HYSTERIA2_SALAMANDER_FIX.md)。

## V1.6.23 主要变化

- 修复把完整代理 URLTest 总耗时直接显示成“节点延迟”的指标混用：节点列表和仪表盘
  显示三次节点服务器 TCP 采样的中位 RTT。
- 真实代理验证没有删除；Core API 同时返回代理传输建立耗时和完整 TLS/HTTP 链路耗时，
  只有完整探针成功才标记“代理可用”。
- 节点详情和连接日志同时记录节点 RTT、代理传输建立、完整代理探针，便于区分节点远、
  WS/TLS 建链慢和目标测试站响应慢。

详细说明见 [V1.6.23_LATENCY_METRICS_FIX.md](V1.6.23_LATENCY_METRICS_FIX.md)。
...
默认运行包输出到项目上级目录：`Hongda-Starlink-V1.6.23-Windows-x64.zip`。详细步骤见 [BUILD_WINDOWS.md](BUILD_WINDOWS.md)。
