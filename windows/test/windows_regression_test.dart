...
import 'package:hongda_starlink/src/subscription_service.dart';
...
class _TestStorageProtector implements StorageProtector {
  @override
...
void main() {
  ...
  test(
    'enables bounded automatic reconnect by default and persists opt-out',
    () {
      expect(
        AppSettings.fromJson(const <String, dynamic>{}).autoReconnect,
        isTrue,
      );
      final disabled = AppSettings(autoReconnect: false);
      expect(AppSettings.fromJson(disabled.toJson()).autoReconnect, isFalse);
    },
  );
  ...
  test('builds only protocols implemented by HongdaCore 1.10', () {
    ...
    expect(
      ...
      isNot(contains('cache_file')),
    );
  });

  test('includes Hysteria2 Salamander subscription nodes', () {
    final temp = Directory.systemTemp.createTempSync('hongda-salamander-test-');
    addTearDown(() => temp.deleteSync(recursive: true));
    final builder = SingBoxConfigBuilder(AppStorage.forDirectory(temp));
    final selected = _node();
    final salamander = NodeProfile(
      id: 'hy2-salamander',
    ...
    expect(SingBoxConfigBuilder.supportsNode(salamander), isTrue);
    final config = builder.build(
      selectedNode: selected,
      nodes: <NodeProfile>[selected, salamander],
      groups: const <ProxyGroupProfile>[],
      rules: const <RouteRuleProfile>[],
      settings: AppSettings(mixedPort: 17891, apiPort: 19091),
    );
    final tags = (config['outbounds'] as List<dynamic>)
        ...
        .map((item) => item['tag']?.toString())
        .toList();
    expect(tags, contains(SingBoxConfigBuilder.nodeTag(salamander.id)));
  });

  test('skips incompatible unselected nodes before Core doctor', () {
    final temp = Directory.systemTemp.createTempSync('hongda-skip-node-test-');
    addTearDown(() => temp.deleteSync(recursive: true));
    final builder = SingBoxConfigBuilder(AppStorage.forDirectory(temp));
    final selected = _node();
    final incompatible = NodeProfile(
      id: 'hy2-unknown-obfs',
    ...
    expect(
      SingBoxConfigBuilder.unsupportedNodeReason(incompatible),
      contains('future-obfs'),
    );
    final config = builder.build(
      selectedNode: selected,
      nodes: <NodeProfile>[selected, incompatible],
      groups: const <ProxyGroupProfile>[],
      rules: const <RouteRuleProfile>[],
      settings: AppSettings(mixedPort: 17891, apiPort: 19091),
    );
    ...
    expect(
      tags,
      isNot(contains(SingBoxConfigBuilder.nodeTag(incompatible.id))),
    );
  });

  test('explains why an incompatible selected node cannot start', () {
    final temp = Directory.systemTemp.createTempSync('hongda-selected-test-');
    addTearDown(() => temp.deleteSync(recursive: true));
    final builder = SingBoxConfigBuilder(AppStorage.forDirectory(temp));
    final incompatible = NodeProfile(
      id: 'tuic-zero-rtt',
    ...
    expect(
      () => builder.build(
        selectedNode: incompatible,
        nodes: <NodeProfile>[incompatible],
        groups: const <ProxyGroupProfile>[],
        rules: const <RouteRuleProfile>[],
        settings: AppSettings(mixedPort: 17891, apiPort: 19091),
      ),
  ...
  test(
    'rejects insecure HTTP subscriptions unless explicitly enabled',
    () async {
      final subscription = SubscriptionProfile(
        id: 'sub-http',
        name: 'insecure',
        url: 'http://127.0.0.1/subscription',
      );
      await expectLater(
        SubscriptionService.fetch(subscription),
        throwsA(isA<FormatException>()),
      );
      subscription.allowInsecureHttp = true;
      expect(subscription.toJson()['allowInsecureHttp'], isTrue);
    },
  );

  test('rotates client logs and removes runtime config copies', () async {
    final temp = Directory.systemTemp.createTempSync('hongda-storage-test-');
    addTearDown(() => temp.deleteSync(recursive: true));
    final storage = AppStorage.forDirectory(temp);

    await storage.appendClientLog(
    ...
    await storage.appendClientLog(
      'third-cccccccccccccccccccccccc',
      maxBytes: 32,
      retainedFiles: 2,
    );
    expect(await storage.clientLogFile.readAsString(), contains('third-'));
    expect(
      await File('${storage.clientLogFile.path}.1').readAsString(),
      contains('second-'),
    );
    expect(
      await File('${storage.clientLogFile.path}.2').readAsString(),
      contains('first-'),
    );

    final config = await storage.writeRuntimeConfig(<String, dynamic>{'a': 1});
    await storage.writeRuntimeConfig(<String, dynamic>{'a': 2});
    expect(await config.exists(), isTrue);
    await storage.deleteRuntimeConfig();
    expect(await config.exists(), isFalse);
    expect(await File('${config.path}.bak').exists(), isFalse);
    expect(await File('${config.path}.tmp').exists(), isFalse);
  });

  test('protects sensitive JSON and migrates legacy plaintext', () async {
    final temp = Directory.systemTemp.createTempSync('hongda-dpapi-test-');
    addTearDown(() => temp.deleteSync(recursive: true));
    final storage = AppStorage.forDirectory(
      temp,
      protector: _TestStorageProtector(),
    );
    final node = _node();
    await storage.saveNodes(<NodeProfile>[node]);
    final protectedNodes = await storage.nodesFile.readAsString();
    expect(protectedNodes, contains('hongda-dpapi-v1'));
    expect(protectedNodes, isNot(contains(node.outbound['uuid'].toString())));
    expect(
      await File('${storage.nodesFile.path}.bak').readAsString(),
      isNot(contains(node.outbound['uuid'].toString())),
    );
    expect(
      (await storage.loadNodes()).single.outbound['uuid'],
      node.outbound['uuid'],
    );

    final legacy = SubscriptionProfile(
      id: 'legacy-sub',
      name: 'legacy',
      url: 'https://token.example/subscription',
    );
    await storage.subscriptionsFile.writeAsString(
      prettyJson(<Map<String, dynamic>>[legacy.toJson()]),
    );
    expect((await storage.loadSubscriptions()).single.url, legacy.url);
    final migrated = await storage.subscriptionsFile.readAsString();
    expect(migrated, contains('hongda-dpapi-v1'));
    expect(migrated, isNot(contains(legacy.url)));
    expect(
      await File('${storage.subscriptionsFile.path}.bak').readAsString(),
      isNot(contains(legacy.url)),
    );
  });
  ...
  test('uses Karing-style China/download direct with proxy final fallback', () {
    ...
    final route = Map<String, dynamic>.from(config['route'] as Map);
    final remoteSets = (route['rule_set'] as List<dynamic>)
        .whereType<Map>()
        .toList();
    final ruleSets = remoteSets.map((item) => item['tag']?.toString()).toList();
    ...
    expect(
      remoteSets.every((item) => item['download_optional'] == true),
      isTrue,
    );
    expect(
      remoteSets.every(
        (item) =>
            item['download_fallback_url'] is List &&
            (item['download_fallback_url'] as List).length == 2,
      ),
      isTrue,
    );
  ...
  test(
    ...
    () async {
      ...
      expect(result.connectDelayMs, 32);
  ...
  test('shows endpoint RTT without discarding full proxy URLTest timing', () {
    final metrics = ProxyDelayMetrics.fromJson(<String, dynamic>{
      'delay': 1091,
      'totalDelay': 1091,
      'connectDelay': 618,
    });

    expect(displayedNodeLatency(endpointRttMs: 212, proxy: metrics), 212);
    expect(metrics.connectDelayMs, 618);
    expect(metrics.totalDelayMs, 1091);
    expect(displayedNodeLatency(endpointRttMs: null, proxy: metrics), 618);
  });
