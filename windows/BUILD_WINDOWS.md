...
运行时释放到 `%LOCALAPPDATA%\HongdaStarlink\runtime\1.10.10\`，执行配置与远程规则集检查，再把 Core 加入 kill-on-close Windows Job Object。
...
# 构建并验证 HongdaService.exe 1.5.10
...
```text
..\Hongda-Starlink-V1.6.23-Windows-x64.zip
```
...
9. 采集 /traffic 和 /connections；连接历史按 /connection-log 游标增量合并
```
