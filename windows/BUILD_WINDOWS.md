...
```text
Hongda Starlink.exe V1.6.9
  -> HongdaService.exe 1.5.0
  -> HongdaCore.exe 1.10.0
...
运行时释放到 `%LOCALAPPDATA%\HongdaStarlink\runtime\1.10.0\`，执行配置与远程规则集检查，再把 Core 加入 kill-on-close Windows Job Object。
...
```text
dist\Hongda-Starlink-V1.6.9-Windows-x64.zip
```
