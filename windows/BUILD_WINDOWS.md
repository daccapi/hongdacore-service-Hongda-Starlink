...
```text
Hongda Starlink.exe V1.6.20
  -> HongdaService.exe 1.5.7
  -> HongdaCore.exe 1.10.7
...
运行时释放到 `%LOCALAPPDATA%\HongdaStarlink\runtime\1.10.7\`，执行配置与远程规则集检查，再把 Core 加入 kill-on-close Windows Job Object。
...
```text
..\Hongda-Starlink-V1.6.20-Windows-x64.zip
```
