...
const (
	serviceVersion = "1.5.7"
	coreVersion    = "1.10.7"
)

const featureSummary = "direct,vless,trojan,hysteria2,tuic,reality,utls,ws,grpc,vless-vision,vless-xudp,rules,remote-rule-set,rule-set-cache,rule-set-fallback,rule-set-offline-start,doh,doh-keepalive,dns-hijack,tcp-dns-hijack,dns-rule-reject,dns-route-cache,dns-query-cache,tls-sni-sniff,udp-associate,udp-proxy,mixed,socks5,http-connect,tun,wintun,gvisor,strict-route,tun-restart-recovery,interface-bind,clash-api,connection-log,connection-log-delta,connection-stream"
