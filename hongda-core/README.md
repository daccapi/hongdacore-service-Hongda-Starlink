...
- `hysteria2` 出站（QUIC / HTTP3 认证，TCP + UDP datagram，支持 Salamander UDP 混淆）
...
- Hysteria2 Salamander 数据包变换、随机盐和密码隔离测试通过。
- 外部配置导入支持 `obfs.type=salamander`，缺少密码和未知混淆仍会明确拒绝。
- 普通与 `with_gvisor` 自动测试及 Windows 构建通过；真实服务器互通需由发布包现场验证。

### 1.10.10 验证结果

- 延迟 API 的代理传输建立、完整 URLTest 分阶段计时与兼容字段回归通过。
- HEAD 测速与禁止重定向不会改变成功状态和 Clash `delay` 字段语义。
- 普通与 `with_gvisor` 全量测试、静态检查和 Windows 构建通过。

### 1.10.9 验证结果

- DPAPI DNS TTL 缓存跨 Router 重建命中，并正确恢复新查询 transaction ID。
- URLTest 阈值保持、实质优势切换、当前失败切换和全失败保留回归通过。
- 普通与 `with_gvisor` 全量测试、静态检查和 Windows 构建通过。
...
以下能力在 1.10.10 **尚未实现**，不能用功能声明或配置导入伪装为可用：
