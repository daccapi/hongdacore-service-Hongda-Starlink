...
- TUN UDP 53 劫持到配置的 DoH detour；没有加密 DNS 时拒绝 auto-route
...
- 已加入真实系统 TUN 测试：系统 DNS、无显式代理的 HTTPS、流量计数和退出回滚。
- 当前测试机仍有 Karing TUN 默认路由，保护逻辑已正确阻止双 TUN 测试；因此 RC
  尚未完成最终实机验收，暂不替换正式客户端内核。
...
以下能力在 1.9.0-rc.1 **尚未实现**，不能用功能声明或配置导入伪装为可用：

- 远程 `.srs` 规则集、规则集匹配、FakeIP、TCP DNS 劫持与进程路由
- WireGuard / Tailscale

因此，当前 Windows 客户端仍应使用已有稳定内核；1.9.0-rc.1 只用于无竞争 TUN
环境的验收。远程规则集和最终 TUN 泄漏测试完成前不要整体替换生产 Core。
...
5. ~~Windows TUN 网卡接管、自动路由与 strict route~~（RC 已实现，待实机验收）
6. ~~UDP DNS 劫持到 DoH~~；继续完成 TCP DNS、FakeIP 与规则 DNS
7. 远程规则集与高级路由
