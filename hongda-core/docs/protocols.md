...
- 写路径先聚合完整 TLS record，避免代理转发将 ClientHello 拆包后破坏 Vision 状态
- TLS ApplicationData record 使用 `End` 结束 padding，后续为原始内层 TLS 流
- 读路径解析服务端 padding 帧；收到 `Direct` 后切换到外层 Reality TLS 的底层
  TCP，并通过 record 边界包装避免 uTLS 预读丢失首段原始数据
- VLESS Addons 使用 protobuf `Flow=1` 字段编码，响应 Addons 会被正确丢弃

## VLESS XUDP

`protocol/xudp.go` 实现了 Mux.Cool UDP（XUDP）客户端：
