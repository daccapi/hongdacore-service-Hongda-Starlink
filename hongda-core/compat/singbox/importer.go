...
func convertNode(tag, typ string, om map[string]interface{}) (model.Node, error) {
	...
	case "hysteria2":
		...
		if obfs, ok := om["obfs"].(map[string]interface{}); ok && strings.TrimSpace(getString(obfs, "type")) != "" {
			return model.Node{}, fmt.Errorf("hysteria2 obfs %q is not implemented", getString(obfs, "type"))
		}
