...
func Import(raw map[string]interface{}) (*model.Config, error) {
	...
	if inbounds, ok := raw["inbounds"].([]interface{}); ok {
		for _, ib := range inbounds {
			...
			case "tun":
				return nil, fmt.Errorf("inbound %q: TUN is not implemented in HongdaCore 1.8.x", ic.Tag)
			default:
	...
	if route, ok := raw["route"].(map[string]interface{}); ok {
		if sets, ok := route["rule_set"].([]interface{}); ok && len(sets) > 0 {
			return nil, fmt.Errorf("route.rule_set is not implemented; refusing to ignore %d rule sets", len(sets))
		}
		...
		if rules, ok := route["rules"].([]interface{}); ok {
			for _, r := range rules {
				...
				if _, exists := rm["rule_set"]; exists {
					return nil, fmt.Errorf("route rule_set matching is not implemented")
				}
				if _, exists := rm["process_name"]; exists {
					return nil, fmt.Errorf("route process_name matching is not implemented")
				}
				if getString(rm, "action") == "hijack-dns" {
					return nil, fmt.Errorf("route hijack-dns is not implemented")
				}
