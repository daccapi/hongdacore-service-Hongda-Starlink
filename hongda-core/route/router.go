...
func (r *Router) lookupIPs(ctx context.Context, host string) ([]net.IP, error) {
	if r.dns.Mode == "doh" && r.dns.Server != "" {
		return lookupDoH(ctx, r.dns.Server, host)
	}
...
func (r *Router) Pipe(client, upstream net.Conn) {
	...
	conn := r.conns.Add("tcp", host, dest)
	defer r.conns.Remove(conn.ID)

	up := &countConn{Conn: upstream, counter: &conn.Upload}
	down := &countConn{Conn: client, counter: &conn.Download}
	...
	<-done
	r.traffic.AddUpload(conn.Upload.Load())
	r.traffic.AddDownload(conn.Download.Load())
	_ = client.Close()
	_ = upstream.Close()
}
