...
func TestRouteDecisionDescribesMatchedRule(t *testing.T) {
	r := New("direct", &telemetry.Traffic{}, telemetry.NewConnections())
	r.SetPolicy([]model.Rule{{
		Match:  model.MatchExpr{DomainSuffix: []string{"youtube.com"}},
		Action: model.ActionRoute,
		Target: "proxy",
	}}, model.DNSOptions{Mode: "local"})
	decision, err := r.routeDecision(context.Background(), "tcp", "www.youtube.com:443")
	if err != nil {
		t.Fatal(err)
	}
	if decision.target != "proxy" || decision.rule != "规则 1 · 域名" {
