...
func (m *windowsManager) Start() error {
	...
	if err != nil && staleTUNAddressError(err) {
		// A force-killed Wintun session can leave the requested address on the
		// persistent adapter even after its routes disappear. sing-tun then
		// fails the next start with ERROR_OBJECT_ALREADY_EXISTS. Remove only the
		// exact HongdaTun addresses from the active store and retry once.
		cleanupStaleTUNAddresses(options)
...
func staleTUNAddressError(err error) bool {
	if err == nil {
		return false
	}
	message := strings.ToLower(err.Error())
	return strings.Contains(message, "object already exists") ||
		strings.Contains(message, "error_object_already_exists")
}

func cleanupStaleTUNAddresses(options tun.Options) {
	name := strings.TrimSpace(options.Name)
	if name == "" {
		return
	}
	remove := func(family, key string, prefixes []netip.Prefix) {
		for _, prefix := range prefixes {
			ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
			command := exec.CommandContext(ctx, "netsh", "interface", family, "delete", "address",
				key+"="+name, "address="+prefix.Addr().String(), "store=active")
			_, _ = command.CombinedOutput()
			cancel()
		}
	}
	remove("ipv4", "name", options.Inet4Address)
	remove("ipv6", "interface", options.Inet6Address)
}
