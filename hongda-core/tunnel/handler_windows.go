...
func (h *handler) NewConnectionEx(ctx context.Context, conn net.Conn, _ M.Socksaddr, destination M.Socksaddr, onClose N.CloseHandlerFunc) {
	if destination.Port == 53 {
		err := h.router.ServeDNSStream(ctx, conn)
		_ = conn.Close()
		if onClose != nil {
			onClose(err)
		}
	...
	if destination.Port == 443 {
		conn, domain = route.SniffTLSHost(conn, 500*time.Millisecond)
	}
	upstream, err := h.router.DialWithDomain("tcp", destination.String(), domain)
	if err != nil {
		_ = conn.Close()
		if onClose != nil {
			onClose(err)
