...
func dialStandardTLSWithNextProtos(ctx context.Context, host string, port uint16, opts *model.TLSOptions, nextProtos []string) (net.Conn, error) {
	...
	// Keep outer TLS reads on record boundaries. uTLS intentionally reads
	// beyond the current record into an internal private buffer; that loses the
	// first raw bytes when Vision CommandDirect switches to the underlying
	// connection. The boundary wrapper avoids read-ahead without reflection or
	// unsafe access to uTLS internals.
	raw = newTLSRecordConn(raw)
...
func newTLSRecordConn(conn net.Conn) net.Conn {
...
func (c *tlsRecordConn) Read(p []byte) (int, error) {
	...
	if len(c.pending) == 0 {
		...
		copy(record, header)
		if _, err := io.ReadFull(c.Conn, record[5:]); err != nil {
			return 0, err
		}
		c.pending = record
	}
...
func dialRealityTLSWithNextProtos(ctx context.Context, host string, port uint16, opts *model.TLSOptions, nextProtos []string) (net.Conn, error) {
	...
	// REALITY authenticates a ClientHello whose session-id field is first
	// cleared to 32 zero bytes. Using the fingerprint's random session ID in
	// AEAD additional data makes the server reject the authentication block and
	// forward the connection to the decoy website.
	...
	copy(originalHello[realitySessionIDOffset:realitySessionIDOffset+32], hello.SessionId)
