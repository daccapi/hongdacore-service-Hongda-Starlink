...
func TestProxyDelayReportsTransportAndTotalPhases(t *testing.T) {
	...
	stats := &telemetry.Traffic{}
	connections := telemetry.NewConnections()
	router := route.New("delayed", stats, connections)
	router.Register(&delayedOutbound{
		Outbound: newTCPOutbound("delayed"),
	...
	endpoint := "http://" + controller.listener.Addr().String() +
		"/proxies/delayed/delay?timeout=1000&url=" + url.QueryEscape(target.URL)
	...
	var payload struct {
		...
		ConnectDelay int64 `json:"connectDelay"`
		Status       int   `json:"status"`
	}
	if err := json.NewDecoder(response.Body).Decode(&payload); err != nil {
		t.Fatal(err)
	}
	if payload.Status != http.StatusNoContent || payload.Delay != payload.TotalDelay {
		t.Fatalf("delay payload = %#v", payload)
	}
	if payload.ConnectDelay < 15 || payload.Delay < payload.ConnectDelay+20 {
		t.Fatalf("delay phases were not separated: %#v", payload)
	}
}
...
func TestConnectionLogCursorReturnsOnlyChangedHistory(t *testing.T) {
	stats := &telemetry.Traffic{}
	controller := newTestController(t, model.APIConfig{Listen: "127.0.0.1:0"}, stats)
	defer controller.Close()
	connection := controller.connRegistry.AddDetailed(telemetry.ConnectionMetadata{
		Network: "tcp", Destination: "203.0.113.8:443", Domain: "cursor.example",
		Route: "proxy", Outbound: "node-test",
	}, func() {})

	base := "http://" + controller.listener.Addr().String() + "/connection-log?limit=10"
	firstResponse, err := http.Get(base)
	if err != nil {
		t.Fatal(err)
	}
	var first struct {
	...
	_ = firstResponse.Body.Close()
	if first.Cursor == 0 || len(first.Connections) != 1 || first.Connections[0]["status"] != "active" {
		t.Fatalf("unexpected first delta: %#v", first)
	}

	controller.connRegistry.Remove(connection.ID)
	secondResponse, err := http.Get(base + "&after=" + strconv.FormatUint(first.Cursor, 10))
	if err != nil {
		t.Fatal(err)
	}
	...
	if second.Cursor <= first.Cursor || len(second.Connections) != 1 || second.Connections[0]["status"] != "closed" {
		t.Fatalf("unexpected closure delta: %#v", second)
	}
}
...
func (d *delayedOutbound) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
...
func newTCPOutbound(id string) *tcpOutbound { return &tcpOutbound{id: id} }

func (o *tcpOutbound) ID() string               { return o.id }
func (o *tcpOutbound) Type() model.ProtocolType { return model.ProtocolDirect }
func (o *tcpOutbound) SupportUDP() bool         { return false }
func (o *tcpOutbound) Close() error             { return nil }
func (o *tcpOutbound) DialContext(ctx context.Context, network, address string) (net.Conn, error) {
	return (&net.Dialer{}).DialContext(ctx, network, address)
}
func (o *tcpOutbound) ListenPacket(context.Context, string) (net.PacketConn, error) {
