...
func Load(ctx context.Context, config model.RulesetConfig, cacheDir string, dialContext DialContext) (*Set, error) {
	...
	data, err := download(ctx, sourceURL(config), dialContext)
	if err != nil {
		if cachedSet != nil {
			return cachedSet, nil
		}
		return nil, fmt.Errorf("download rule set %q: %w", config.ID, err)
	}
	set, err := ParseSource(config.ID, data)
	if err != nil {
		if cachedSet != nil {
			return cachedSet, nil
		}
...
func sourceURL(config model.RulesetConfig) string {
