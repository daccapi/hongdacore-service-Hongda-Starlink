...
// It embeds the official sing-box 1.13.18 Windows amd64 runtime and extracts
// it into the user's local application data directory on demand. This keeps
// the release self-contained while preserving the upstream sing-box binary.
package main
...
const featureSummary = "vless,reality,hysteria2,tuic,anytls,tailscale,clash-api,wireguard,acme,dhcp,gvisor,naive,ccm,ocm"

//go:embed embedded/sing-box.exe
var coreBinary []byte

//go:embed embedded/libcronet.dll
var cronetBinary []byte
...
func usage() {
	fmt.Fprintln(os.Stderr, "HongdaService - bundled sing-box supervisor")
...
func ensureRuntime() (runtimeFiles, error) {
	if len(coreBinary) < 10*1024*1024 {
		return runtimeFiles{}, errors.New("embedded sing-box core is missing or truncated")
	}
	...
	corePath := filepath.Join(dir, "sing-box.exe")
	cronetPath := filepath.Join(dir, "libcronet.dll")
	coreSHA := sha256Hex(coreBinary)
	if err := writeEmbeddedIfNeeded(corePath, coreBinary, coreSHA, 0o755); err != nil {
		return runtimeFiles{}, fmt.Errorf("extract sing-box core: %w", err)
	}
	cronetSHA := sha256Hex(cronetBinary)
	if err := writeEmbeddedIfNeeded(cronetPath, cronetBinary, cronetSHA, 0o644); err != nil {
		return runtimeFiles{}, fmt.Errorf("extract libcronet: %w", err)
	}
	return runtimeFiles{
		Dir:       dir,
		Core:      corePath,
...
func printVersion() error {
	...
	if err != nil {
		return fmt.Errorf("query sing-box version: %w: %s", err, compact(string(out)))
	}
	line := firstLine(string(out))
	fmt.Printf("HongdaService %s · %s · embedded core sha256:%s\n", serviceVersion, line, files.CoreSHA[:12])
	return nil
}
...
func runCheck(files runtimeFiles, absolute string) error {
	...
	if err != nil {
		...
		frame := errorFrame{Phase: "config_check", Message: "sing-box configuration check failed", ExitCode: code, Detail: detail}
...
func runService(config string) error {
	...
	if jobErr != nil {
		...
		frame := errorFrame{
			...
			Message: "failed to attach sing-box to kill-on-close Job Object",
			Detail:  jobErr.Error(),
		}
	...
	go forward(childStdout, os.Stderr, "[sing-box] ")
	go forward(childStderr, os.Stderr, "[sing-box] ")
	...
	case waitErr := <-exited:
		if waitErr != nil {
			...
			frame := errorFrame{Phase: "core_exit", Message: "sing-box exited unexpectedly", ExitCode: code, Detail: waitErr.Error()}
			return reportError(frame, fmt.Errorf("sing-box exited with code %d: %w", code, waitErr))
		}
		return nil
	case <-signals:
		requestStop()
	...
	// The official Windows core is supervised as our child and is assigned to a
	// kill-on-close Windows Job Object. If HongdaService itself is terminated,
	// the OS closes the job handle and sing-box is terminated with it.
	cancel()
	if cmd.Process != nil {
		_ = cmd.Process.Kill()
	}
...
func ipcLoop(corePID int, config string, files runtimeFiles, startedAt time.Time, requestStop func(), mu *sync.Mutex) {
	...
	// supervised sing-box process cannot remain orphaned.
	requestStop()
}
