...
class ConfigImporter {
  static ConfigImportResult parse(String content, {String source = 'manual'}) {
    ...
    if (text.startsWith('{') || text.startsWith('[')) {
      try {
        ...
        if (decoded is Map) {
          final result = _parseSingBox(Map<String, dynamic>.from(decoded), source);
          if (result.nodes.isNotEmpty || result.groups.isNotEmpty || result.rules.isNotEmpty) {
            return result;
          }
        }
      } catch (_) {
    ...
    if (RegExp(r'(?m)^\s*(proxies|proxy-groups|rules)\s*:').hasMatch(text)) {
      try {
        ...
        if (yaml is YamlMap) {
          final result = _parseClash(_plain(yaml) as Map<String, dynamic>, source);
          if (result.nodes.isNotEmpty || result.groups.isNotEmpty || result.rules.isNotEmpty) {
            return result;
          }
        }
  ...
  static ConfigImportResult _parseSingBox(Map<String, dynamic> root, String source) {
    ...
    resolveGroupMembers = (String tag, Set<String> visiting) {
      ...
      final result = <String>{};
      for (final memberTag in (raw['outbounds'] as List?)?.map((e) => e.toString()) ?? const <String>[]) {
    ...
    for (final raw in groupDefs) {
      ...
      groups.add(ProxyGroupProfile(
        id: groupId,
        name: tag,
        type: raw['type']?.toString() == 'urltest' ? 'urltest' : 'selector',
        nodeIds: memberIds,
        selectedNodeId: selectedNodeId,
        testUrl: raw['url']?.toString() ?? 'https://www.gstatic.com/generate_204',
    ...
    final rawRules = ((root['route'] as Map?)?['rules'] as List?) ?? const <dynamic>[];
    var index = 0;
    for (final raw in rawRules) {
      ...
      if (tagToGroupId.containsKey(outbound)) outbound = 'group:${tagToGroupId[outbound]}';
      if (tagToNode.containsKey(outbound)) outbound = 'node:${tagToNode[outbound]!.id}';
    ...
    if (route is Map) {
      ...
      if (finalTarget.isNotEmpty) {
        ...
        } else if (finalTarget != 'direct' && finalTarget != 'block' &&
            finalTarget != 'proxy' && finalTarget != 'auto') {
          finalTarget = 'proxy';
        }
        rules.add(RouteRuleProfile(
    ...
    return ConfigImportResult(nodes: _dedupe(nodes), groups: groups, rules: rules, format: 'sing-box');
  }

  static ConfigImportResult _parseClash(Map<String, dynamic> root, String source) {
    final nodes = <NodeProfile>[];
    ...
    for (final raw in rawGroups) {
      ...
      if (type != 'select' && type != 'url-test' && type != 'fallback') continue;
    ...
    for (final entry in groupByName.entries) {
      ...
      groups.add(ProxyGroupProfile(
        ...
        testUrl: map['url']?.toString() ?? 'https://www.gstatic.com/generate_204',
        intervalSeconds: int.tryParse(map['interval']?.toString() ?? '') ?? 180,
    ...
    return ConfigImportResult(nodes: _dedupe(nodes), groups: groups, rules: rules, format: 'clash');
  }
  ...
  static NodeProfile? _clashProxy(Map<String, dynamic> p, String source) {
    ...
    final out = <String, dynamic>{'type': type, 'server': server, 'server_port': port};
    switch (type) {
      case 'vless':
        out['uuid'] = p['uuid']?.toString() ?? '';
        if ((p['flow']?.toString() ?? '').isNotEmpty) out['flow'] = p['flow'].toString();
      ...
      case 'vmess':
        ...
        out['alter_id'] = int.tryParse(p['alterId']?.toString() ?? p['alter-id']?.toString() ?? '') ?? 0;
        _clashTls(out, p);
        _clashTransport(out, p);
        break;
      case 'trojan':
      ...
      case 'shadowsocks':
        ...
        break;
      case 'hysteria2':
        out['password'] = p['password']?.toString() ?? p['auth']?.toString() ?? '';
        _clashTls(out, p, force: true);
        final obfs = p['obfs']?.toString() ?? '';
        if (obfs.isNotEmpty) {
          out['obfs'] = <String, dynamic>{
            'type': obfs,
            if ((p['obfs-password']?.toString() ?? '').isNotEmpty) 'password': p['obfs-password'].toString(),
          };
        }
        break;
      case 'tuic':
        out['uuid'] = p['uuid']?.toString() ?? '';
        out['password'] = p['password']?.toString() ?? '';
        if ((p['congestion-controller']?.toString() ?? '').isNotEmpty) out['congestion_control'] = p['congestion-controller'].toString();
        if ((p['udp-relay-mode']?.toString() ?? '').isNotEmpty) out['udp_relay_mode'] = p['udp-relay-mode'].toString();
        _clashTls(out, p, force: true);
        break;
      case 'anytls':
        out['password'] = p['password']?.toString() ?? '';
      ...
      case 'socks':
      case 'http':
        if ((p['username']?.toString() ?? '').isNotEmpty) out['username'] = p['username'].toString();
        if ((p['password']?.toString() ?? '').isNotEmpty) out['password'] = p['password'].toString();
  ...
  static void _clashTls(Map<String, dynamic> out, Map<String, dynamic> p, {bool force = false, bool reality = false}) {
    ...
    if (fp.isNotEmpty) tls['utls'] = <String, dynamic>{'enabled': true, 'fingerprint': fp};
    ...
    if (realityOpts is Map) {
      ...
      tls['reality'] = <String, dynamic>{
        'enabled': true,
        if ((r['public-key']?.toString() ?? '').isNotEmpty) 'public_key': r['public-key'].toString(),
        if ((r['short-id']?.toString() ?? '').isNotEmpty) 'short_id': r['short-id'].toString(),
      };
    }
    out['tls'] = tls;
  }

  static void _clashTransport(Map<String, dynamic> out, Map<String, dynamic> p) {
    final network = p['network']?.toString().toLowerCase() ?? '';
    if (network == 'ws') {
      final opts = p['ws-opts'] is Map ? Map<String, dynamic>.from(p['ws-opts'] as Map) : const <String, dynamic>{};
      final headers = opts['headers'] is Map ? Map<String, dynamic>.from(opts['headers'] as Map).map((k, v) => MapEntry(k, v.toString())) : <String, String>{};
      out['transport'] = <String, dynamic>{
        'type': 'ws',
        if ((opts['path']?.toString() ?? '').isNotEmpty) 'path': opts['path'].toString(),
        if (headers.isNotEmpty) 'headers': headers,
      };
    } else if (network == 'grpc') {
      final opts = p['grpc-opts'] is Map ? Map<String, dynamic>.from(p['grpc-opts'] as Map) : const <String, dynamic>{};
      out['transport'] = <String, dynamic>{
        'type': 'grpc',
        if ((opts['grpc-service-name']?.toString() ?? '').isNotEmpty) 'service_name': opts['grpc-service-name'].toString(),
      };
  ...
  static String _clashTarget(String target, Map<String, String> groupNameToId, Map<String, NodeProfile> nameToNode) {
    final upper = target.toUpperCase();
    if (upper == 'DIRECT') return 'direct';
    if (upper == 'REJECT' || upper == 'REJECT-DROP') return 'block';
    if (groupNameToId.containsKey(target)) return 'group:${groupNameToId[target]}';
    if (nameToNode.containsKey(target)) return 'node:${nameToNode[target]!.id}';
    return 'proxy';
  }

  static bool _hasMatchers(RouteRuleProfile r) =>
      r.domains.isNotEmpty || r.domainSuffixes.isNotEmpty || r.domainKeywords.isNotEmpty ||
      r.ipCidrs.isNotEmpty || r.processNames.isNotEmpty || r.network.isNotEmpty;
  ...
  static dynamic _plain(dynamic value) {
    if (value is YamlMap) return <String, dynamic>{for (final e in value.entries) e.key.toString(): _plain(e.value)};
  ...
  static const Set<String> _proxyTypes = <String>{
    ...
    'tuic', 'socks', 'http', 'naive', 'shadowtls', 'anytls'
  };
}
