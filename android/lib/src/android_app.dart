...
class _AndroidMainShellState extends State<AndroidMainShell> {
  ...
  void initState() {
    ...
    controller = AndroidRuntimeController(
      storage: storage,
      trafficStats: trafficStats,
    )..addListener(_onRuntimeChanged);
    unawaited(storage.saveSettings(settings));
    unawaited(controller.initialize(settings: settings));
  ...
  Future<void> _toggleConnection() async {
    if (controller.isRunning ||
        controller.status == AndroidConnectionStatus.preparing) {
  ...
  Future<void> _updateSubscriptions() async {
    ...
    try {
      for (final sub in subscriptions) {
        try {
          ...
          final previous = nodes
              .where((n) => n.source == sub.id)
              .toList(growable: false);
          final oldByFingerprint = <String, NodeProfile>{
            for (final n in previous) n.fingerprint: n,
          };
          final oldByIdentity = <String, NodeProfile>{
            for (final n in previous) n.identityKey: n,
          };
          final endpointCounts = <String, int>{};
          final oldByEndpoint = <String, NodeProfile>{};
          for (final old in previous) {
            endpointCounts[old.endpointKey] =
                (endpointCounts[old.endpointKey] ?? 0) + 1;
            oldByEndpoint[old.endpointKey] = old;
          }
          ...
          for (final node in result.imported.nodes) {
            final old =
                oldByFingerprint[node.fingerprint] ??
                oldByIdentity[node.identityKey] ??
                (endpointCounts[node.endpointKey] == 1
                    ? oldByEndpoint[node.endpointKey]
                    : null);
            if (old != null) {
              node.id = old.id;
      ...
      if (settings.selectedNodeId.isEmpty ||
          !nodes.any((n) => n.id == settings.selectedNodeId)) {
  ...
  Future<void> _addManualNodes() async {
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const Text(
              '支持 VLESS / VMess / Trojan / Hysteria2 / Shadowsocks / SOCKS。',
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                hintText: '每行粘贴一个节点链接，或粘贴 Base64 订阅内容',
              ),
            ),
            const SizedBox(height: 14),
            SizedBox(
    ...
    setState(() {
      ...
      settings.selectedNodeId = settings.selectedNodeId.isEmpty
          ? imported.first.id
    ...
    setState(() {
      subscriptions.add(
        SubscriptionProfile(
          ...
          name: name,
          url: url,
          userAgent: 'HongdaStarlink/1.6.5 Android',
        ),
      );
    });
              ...
              onDelete: (sub) async {
                setState(() {
                  ...
                  if (!nodes.any((n) => n.id == settings.selectedNodeId)) {
                    settings.selectedNodeId = nodes.isEmpty
                        ? ''
                        : nodes.first.id;
                  }
                });
              ...
              Row(
                children: <Widget>[
                  Expanded(
                    child: _MetricCard(
                      label: '延迟',
                      value: selectedNode?.latencyMs?.toString() ?? '--',
                      unit: 'ms',
                      icon: Icons.graphic_eq_rounded,
                      accent: const Color(0xFF2AB67D),
                    ),
                  ),
                  ...
                  Expanded(
                    child: _MetricCard(
                      label: '下载',
                      value: _speedValue(controller.downloadBytesPerSecond),
                      unit: _speedUnit(controller.downloadBytesPerSecond),
                      icon: Icons.arrow_downward_rounded,
                      accent: const Color(0xFF1E73F0),
                    ),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: _MetricCard(
                      label: '上传',
                      value: _speedValue(controller.uploadBytesPerSecond),
                      unit: _speedUnit(controller.uploadBytesPerSecond),
                      icon: Icons.arrow_upward_rounded,
                      accent: const Color(0xFF8A58F4),
                    ),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: _MetricCard(
                      label: '总流量',
                      value: _trafficValue(controller.totalTrafficBytes),
                      unit: _trafficUnit(controller.totalTrafficBytes),
                      icon: Icons.pie_chart_rounded,
                      accent: const Color(0xFF1F6FF2),
                    ),
                  ),
                ],
              ),
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: _QuickToggle(
                      ...
                      label: 'DNS',
                      value: settings.dnsMode == 'doh',
                      iconColor: const Color(0xFF27B77C),
                  ...
                  Expanded(
                    child: _QuickToggle(
                      ...
                      label: '全局接管',
                      value: true,
                      iconColor: const Color(0xFF8C5CF5),
                  ...
                  Expanded(
                    child: _QuickToggle(
                      ...
                      label: '自动选择',
                      value: settings.routeMode == 'auto',
                      iconColor: const Color(0xFFFF981F),
      ...
      child: Stack(
        children: <Widget>[
          const Positioned.fill(
            child: CustomPaint(painter: _HeroSpacePainter()),
          ),
          Padding(
                ...
                SizedBox(
                  ...
                  child: Center(
                    child: _ConnectionOrb(
                      ...
                      duration: controller.connectedAt == null
                          ? Duration.zero
                        ...
                        children: <Widget>[
                          ...
                          Expanded(
                            child: Text(
                              connected
                                  ? '连接稳定，网络通畅'
                                  : (controller.coreAvailable
                                        ? '点击左侧按钮开始连接'
                                        : 'UI 已就绪 · Android Core 待接入'),
                              maxLines: 1,
                              overflow: TextOverflow.ellipsis,
                              style: const TextStyle(
    ...
    child: AnimatedBuilder(
      ...
      builder: (context, child) => CustomPaint(
        painter: _ConnectionRingPainter(
          ...
          active: widget.connected || widget.preparing,
        ),
        child: SizedBox(
          width: 132,
          height: 132,
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: const RadialGradient(
                  colors: <Color>[Color(0xFF2F91FF), Color(0xFF1778FF)],
                ),
                boxShadow: const <BoxShadow>[
                  BoxShadow(
                    color: Color(0x664FD4FF),
                    blurRadius: 26,
                    spreadRadius: 2,
                  ),
                ],
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      ...
      children: <Widget>[
        ...
        Text(
          label == '延迟' ? '实时延迟' : (label == '总流量' ? '本次连接' : '当前速度'),
          style: const TextStyle(fontSize: 9.5, color: Color(0xFF9BA7BA)),
        ),
      ],
    ),
  );
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(
      children: <Widget>[
        ...
        Expanded(
          child: CustomPaint(
            painter: _TrafficPainter(download: download, upload: upload),
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(
            colors: <Color>[Color(0xFFF8FBFF), Color(0xFFF2F7FF)],
          ),
          borderRadius: BorderRadius.circular(18),
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                gradient: const LinearGradient(
                  ...
                  colors: <Color>[Color(0xFF8FC2FF), Color(0xFF2E75F6)],
                ),
                boxShadow: const <BoxShadow>[
                  BoxShadow(
                    color: Color(0x332E75F6),
                    blurRadius: 14,
                    offset: Offset(0, 7),
                  ),
                ],
            ...
            Expanded(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Text(
                    latest == null ? '尚未更新' : '最近更新：${_formatDateTime(latest)}',
      ...
      child: Column(
        children: <Widget>[
          for (var i = 0; i < nodes.length; i++) ...<Widget>[
            _RecentNodeRow(
              node: nodes[i],
              selected: selectedNode?.id == nodes[i].id,
              onSelect: () => onSelect(nodes[i]),
              onFavorite: () => onFavorite(nodes[i]),
            ),
            if (i != nodes.length - 1)
    ...
    child: SizedBox(
      ...
      child: Row(
        children: <Widget>[
          ...
          Expanded(
            child: Column(
              ...
              children: <Widget>[
                ...
                const SizedBox(height: 2),
                Text(
                  '${nodeRegion(node).name} · ${node.protocol.toUpperCase()}  |  ${_nodeFeature(node)}',
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: const TextStyle(fontSize: 9.5, color: _muted),
                ),
          ...
          Text(
            node.latencyMs == null ? '-- ms' : '${node.latencyMs} ms',
            style: TextStyle(
              fontSize: 11.5,
              fontWeight: FontWeight.w700,
          ...
          IconButton(
            ...
            icon: Icon(
              ...
              color: node.favorite
                  ? const Color(0xFFFFA91F)
                  : const Color(0xFF8794AA),
              size: 21,
            ),
          ),
        ],
...
class _AndroidNodesPage extends StatefulWidget {
  const _AndroidNodesPage({
    required this.nodes,
    required this.selectedNode,
    required this.testingIds,
    required this.onSelect,
    required this.onFavorite,
  ...
  Widget build(BuildContext context) {
    ...
    final visible = widget.nodes.where((n) {
      ...
      return query.isEmpty ||
          ...
          nodeRegion(n).name.toLowerCase().contains(query);
    }).toList();
    if (filter == 'latency') {
      visible.sort(
        ...
        children: <Widget>[
          ...
          Row(
            children: <Widget>[
              ...
              _FilterChip(
                label: '低延迟',
                selected: filter == 'latency',
                onTap: () => setState(() => filter = 'latency'),
              ),
            ],
          ),
          const SizedBox(height: 12),
          Expanded(
            child: visible.isEmpty
                ? const Center(
                    child: Text('没有匹配的节点', style: TextStyle(color: _muted)),
                  )
                : ListView.separated(
                    physics: const BouncingScrollPhysics(),
  ...
  Widget build(BuildContext context) => InkWell(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: selected ? _blue : const Color(0xFFE1E8F2)),
      ),
      child: Text(
        label,
        style: TextStyle(
          fontSize: 11,
          fontWeight: FontWeight.w700,
          color: selected ? Colors.white : const Color(0xFF52607A),
        ),
      ),
    ),
  );
    ...
    child: InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(17),
          border: Border.all(
            color: selected ? const Color(0xFF99BEFF) : const Color(0xFFE8EEF7),
            width: selected ? 1.4 : 1,
          ),
          boxShadow: const <BoxShadow>[
            BoxShadow(
              color: Color(0x08193B69),
              blurRadius: 12,
              offset: Offset(0, 5),
            ),
          ],
            ...
            InkWell(
              ...
              child: Padding(
                ...
                child: testing
                    ...
                    : Text(
                        node.latencyMs == null ? '测试' : '${node.latencyMs} ms',
                        style: TextStyle(
                          fontSize: 11,
                          fontWeight: FontWeight.w700,
...
class _AndroidSubscriptionsPage extends StatelessWidget {
  const _AndroidSubscriptionsPage({
    required this.subscriptions,
    required this.updating,
  ...
  Widget build(BuildContext context) => _MobilePageScaffold(
    ...
    child: subscriptions.isEmpty
        ? _EmptyState(
            ...
            title: '还没有订阅',
            subtitle: '添加 Clash / sing-box / URI 订阅地址',
            buttonText: '添加订阅',
            onPressed: onAdd,
          )
        : ListView.separated(
                      ...
                      children: <Widget>[
                        ...
                        Text(
                          sub.lastUpdated == null
                              ? '未更新'
                              : _formatDateTime(sub.lastUpdated!),
                          style: const TextStyle(fontSize: 10, color: _muted),
                        ),
                      ],
                    ),
    ...
    child: ListView(
      ...
      children: <Widget>[
        _ToolActionCard(
          ...
          title: '批量延迟测试',
          subtitle: '测试全部启用节点的连接延迟',
          color: const Color(0xFF2E7BF5),
          onTap: onTestAll,
        ),
        const SizedBox(height: 9),
        _ToolActionCard(
          icon: Icons.auto_awesome_rounded,
          title: '自动选择最佳节点',
          subtitle: '测试后自动切换到当前最低延迟节点',
          color: const Color(0xFFFF9C28),
          onTap: onAutoSelect,
        ),
        const SizedBox(height: 14),
        ...
        TextFormField(
          ...
          onFieldSubmitted: (value) {
            settings.dnsPrimary = value.trim();
            onChanged();
          },
          decoration: const InputDecoration(
            labelText: 'DoH 地址',
        ...
        Container(
          padding: const EdgeInsets.all(13),
          decoration: BoxDecoration(
            color: controller.coreAvailable
            ...
            children: <Widget>[
              Icon(
                ...
                color: controller.coreAvailable
                    ...
                    : const Color(0xFFD28B21),
              ),
              const SizedBox(width: 9),
              Expanded(
                child: Text(
                  controller.coreAvailable
                      ? 'Android libbox 核心已检测到'
                      : '当前源码未打包 libbox.aar，连接功能会给出明确提示，不会再显示 Windows Service 异常。',
                  style: const TextStyle(
                    fontSize: 10.5,
                    color: Color(0xFF5C667B),
                    height: 1.4,
                  ),
                ),
              ),
    ...
    child: ListView(
      ...
      children: <Widget>[
        Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(
              colors: <Color>[Color(0xFF176BFF), Color(0xFF3588FF)],
            ),
            borderRadius: BorderRadius.circular(20),
          ),
          child: Row(
              ...
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    const SizedBox(height: 3),
                    const Text(
                      'Android Edition · v1.6.5',
                      style: TextStyle(color: Color(0xFFD9E8FF), fontSize: 11),
                    ),
                    const SizedBox(height: 4),
                    Text(
                      coreAvailable ? 'libbox runtime ready' : 'UI / Data mode',
                      style: const TextStyle(
        ...
        _SettingSwitchTile(
          ...
          title: 'FakeIP',
          subtitle: '按规则使用 FakeIP DNS',
          value: settings.fakeIpEnabled,
          onChanged: (value) {
            settings.fakeIpEnabled = value;
            onChanged();
          },
        ),
        const SizedBox(height: 12),
        ...
        TextFormField(
          ...
          onFieldSubmitted: (value) {
            settings.urlTestUrl = value.trim();
            onChanged();
          },
            ...
            Container(
              ...
              child: SingleChildScrollView(
                child: SelectableText(
                  logs.isEmpty
                      ? '暂无日志'
                      : logs.reversed.take(80).toList().reversed.join('\n'),
                  style: const TextStyle(
                      ...
                      children: <Widget>[
                        AnimatedContainer(
                          ...
                          child: Icon(
                            items[i].$1,
                            size: 22,
                            color: index == i ? _blue : const Color(0xFF7E8CA7),
                          ),
                        ),
                        const SizedBox(height: 2),
                        Text(
              ...
              child: Row(
                children: <Widget>[
                  Icon(
                    ...
                    size: 18,
                    color: current == entry.key ? _blue : _muted,
                  ),
                  const SizedBox(width: 8),
                  Text(entry.value),
                ],
...
class _ConnectionRingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final point = Offset(
      center.dx + math.cos(glowAngle) * (radius - 7),
      center.dy + math.sin(glowAngle) * (radius - 7),
    );
    if (active) {
      canvas.drawCircle(
        point,
        8,
        Paint()..color = const Color(0xFF7EECFF).withOpacity(.20),
      );
...
class _TrafficPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxValue = math.max(
      1024.0,
      all.isEmpty ? 1024.0 : all.reduce((a, b) => a > b ? a : b),
    );
...
String _formatBytes(int bytes) =>
    '${_trafficValue(bytes)} ${_trafficUnit(bytes)}';

String _nodeFlag(NodeProfile node) => nodeRegion(node).emoji;
...
String _nodeFeature(NodeProfile node) {
  ...
  if (transport is Map && transport['type'] != null)
    return transport['type'].toString().toUpperCase();
