...
class NodeProfile {
  ...
  factory NodeProfile.fromJson(Map<String, dynamic> json) {
    return NodeProfile(
      ...
      latencyMs: json['latencyMs'] == null ? null : _intValue(json['latencyMs']),
      lastLatencyTest: DateTime.tryParse(json['lastLatencyTest']?.toString() ?? ''),
...
class ProxyGroupProfile {
  ...
  factory ProxyGroupProfile.fromJson(Map<String, dynamic> json) => ProxyGroupProfile(
        id: json['id']?.toString() ?? '',
        name: json['name']?.toString() ?? '代理组',
        type: json['type']?.toString() == 'urltest' ? 'urltest' : 'selector',
        nodeIds: (json['nodeIds'] as List?)?.map((e) => e.toString()).toList() ?? <String>[],
        selectedNodeId: json['selectedNodeId']?.toString() ?? '',
        testUrl: json['testUrl']?.toString() ?? 'https://www.gstatic.com/generate_204',
        intervalSeconds: int.tryParse(json['intervalSeconds']?.toString() ?? '') ?? 180,
...
class RouteRuleProfile {
  ...
  factory RouteRuleProfile.fromJson(Map<String, dynamic> json) => RouteRuleProfile(
        id: json['id']?.toString() ?? '',
  ...
  static List<String> _stringList(dynamic value) =>
      (value as List?)?.map((e) => e.toString()).where((e) => e.trim().isNotEmpty).toList() ?? <String>[];
}
...
class TrafficStats {
  ...
  void _trimDaily() {
    final keys = <String>{...dailyUploadBytes.keys, ...dailyDownloadBytes.keys}.toList()..sort();
  ...
  factory TrafficStats.fromJson(Map<String, dynamic> json) {
    ...
    return TrafficStats(
      totalUploadBytes: int.tryParse(json['totalUploadBytes']?.toString() ?? '') ?? 0,
      totalDownloadBytes: int.tryParse(json['totalDownloadBytes']?.toString() ?? '') ?? 0,
...
class AppSettings {
  ...
  factory AppSettings.fromJson(Map<String, dynamic> json) {
    return AppSettings(
      ...
      urlTestUrl: json['urlTestUrl']?.toString() ?? 'https://www.gstatic.com/generate_204',
      urlTestIntervalSeconds: int.tryParse(json['urlTestIntervalSeconds']?.toString() ?? '') ?? 180,
      urlTestToleranceMs: int.tryParse(json['urlTestToleranceMs']?.toString() ?? '') ?? 50,
      ...
      tailscaleHostname: json['tailscaleHostname']?.toString() ?? 'hongda-starlink',
      tailscaleAcceptRoutes: json['tailscaleAcceptRoutes'] != false,
      tailscaleExitNode: json['tailscaleExitNode']?.toString() ?? '',
      tailscaleExitNodeAllowLanAccess: json['tailscaleExitNodeAllowLanAccess'] != false,
    );
  }
}
