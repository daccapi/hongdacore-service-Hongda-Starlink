...
class AndroidRuntimeController extends ChangeNotifier {
  ...
  Future<void> start({
    ...
    try {
      ...
      final v4Routes = int.tryParse(nativeStatus['tunRouteV4Count']?.toString() ?? '') ?? 0;
      final v6Routes = int.tryParse(nativeStatus['tunRouteV6Count']?.toString() ?? '') ?? 0;
      ...
      _appendLog('Android 兼容配置：Karing-style · stack=gvisor · MTU=4064 · TUN IPv4-only · sniff+DNS hijack');
      _appendLog('Android TUN：v4=${nativeStatus['tunRouteV4Count'] ?? 0} · v6=${nativeStatus['tunRouteV6Count'] ?? 0} · DNS=${nativeStatus['tunDns'] ?? '--'} · IF=${nativeStatus['defaultInterface'] ?? '--'}');
      final coreRequestedRoutes = nativeStatus['coreRequestedRoutes']?.toString() ?? '';
      _appendLog(
      ...
      final includePackages = nativeStatus['coreRequestedIncludePackages']?.toString() ?? '';
      final excludePackages = nativeStatus['coreRequestedExcludePackages']?.toString() ?? '';
      if (includePackages.isNotEmpty || excludePackages.isNotEmpty) {
        _appendLog('VPN 应用过滤：R9 已强制忽略 · include=$includePackages · exclude=$excludePackages');
      }
  ...
  Future<int?> testNode(NodeProfile node, AppSettings settings) async {
    ...
    try {
      if (isRunning) {
        ...
        try {
          ...
          if (response.statusCode < 200 || response.statusCode >= 300) {
            throw HttpException('Core URLTest ${response.statusCode}: $body', uri: uri);
          }
          final decoded = jsonDecode(body);
          final delay = decoded is Map ? int.tryParse(decoded['delay']?.toString() ?? '') : null;
      ...
      if (protocol == 'hysteria2' || protocol == 'hysteria' || protocol == 'tuic') {
        node.testError = '${node.protocol} 使用 UDP；请连接后由 HongdaCore 执行真实 URLTest';
        return null;
      }
  ...
  Future<void> _runPostConnectDiagnostics(NodeProfile node, AppSettings settings) async {
    ...
    final probeHost = Uri.tryParse(settings.urlTestUrl)?.host.isNotEmpty == true
        ...
        : 'www.gstatic.com';
    try {
      final answer = await InternetAddress.lookup(probeHost).timeout(const Duration(seconds: 6));
      dnsOk = answer.isNotEmpty;
      _appendLog('诊断 TUN DNS：正常 · $probeHost -> ${answer.take(2).map((e) => e.address).join(', ')}');
    } catch (e) {
      final native = await AndroidPlatformBridge.runtimeStatus();
      _appendLog(
    ...
    try {
      ...
      if (uri == null || !uri.hasScheme || uri.host.isEmpty) throw StateError('URLTest 地址无效');
      final client = HttpClient()..connectionTimeout = const Duration(seconds: 5);
      try {
        final request = await client.getUrl(uri);
        final response = await request.close().timeout(const Duration(seconds: 8));
        ...
        _appendLog('诊断完整代理：${fullTunnelOk ? '正常' : '异常'} · HTTP ${response.statusCode}');
      } finally {
        client.close(force: true);
      }
    } catch (e) {
    ...
    if (coreOutboundOk && !ipTunnelOk) {
      _appendLog('诊断结论：节点/Core 出口可用，但 Android 应用流量没有完成 TUN 转发；重点检查 VpnService 路由/设备兼容。', error: true);
    } else if (coreOutboundOk && ipTunnelOk && !dnsOk) {
      _appendLog('诊断结论：IP 代理链路正常，DNS 链路异常。', error: true);
    } else if (!coreOutboundOk) {
      _appendLog('诊断结论：Core 自身无法通过当前节点完成 URLTest，问题在节点握手、物理出口或节点配置。', error: true);
    } else if (coreOutboundOk && ipTunnelOk && dnsOk && !fullTunnelOk) {
      _appendLog('诊断结论：Core/IP/DNS 均正常，但域名 HTTP 仍失败；继续检查 MTU/TLS/规则命中。', error: true);
    } else if (fullTunnelOk) {
      _appendLog('诊断结论：鸿达进程内 TUN + DNS + 节点链路正常；R9 使用 Karing-style Android TUN 参数并强制 GLOBAL 全应用接管。');
    }
    notifyListeners();
  }
