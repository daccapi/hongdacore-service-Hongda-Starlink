    ...
    return Scaffold(
      ...
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final sidebarWidth = collapsedSidebar ? 72.0 : (compact ? 196.0 : 236.0);
          return DecoratedBox(
                          ...
                          Expanded(
                            child: Container(
                              margin: EdgeInsets.fromLTRB(
                                compact ? 9 : 14,
                                page == AppPage.dashboard ? (compact ? 38 : 44) : (compact ? 44 : 54),
                                compact ? 9 : 14,
  ...
  Widget _buildPage() {
    switch (page) {
      ...
      case AppPage.networkTools:
        return _NetworkToolsPage(controller: controller, settings: settings, nodes: nodes, onNodesChanged: () async {
          setState(() {});
          await storage.saveNodes(nodes);
        });
      case AppPage.config:
  ...
  List<NodeProfile> get visibleNodes {
    ...
    var result = widget.nodes.where((node) {
      ...
      return node.name.toLowerCase().contains(query) ||
          ...
          _nodeRegionName(node).toLowerCase().contains(query);
    }).toList();
    if (filter == 'auto') {
      result.sort((a, b) {
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final metrics = <Widget>[
            _HeroMetric(label: '延迟', value: node?.latencyMs == null ? '--' : '${node!.latencyMs} ms', data: const <double>[], dense: dense),
            _HeroMetric(label: '下载', value: _formatSpeed(controller.downloadBytesPerSecond), data: downloadHistory, dense: dense),
            _HeroMetric(label: '上传', value: _formatSpeed(controller.uploadBytesPerSecond), data: uploadHistory, green: true, dense: dense),
            _HeroMetric(label: '流量', value: _formatBytes(controller.totalTrafficBytes), data: downloadHistory, purple: true, dense: dense),
          ];
          ...
          return Padding(
            padding: EdgeInsets.fromLTRB(16, dense ? 12 : 22, 16, dense ? 10 : 14),
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: <Widget>[
                mainContent,
...
class _HeroMetric extends StatelessWidget {
  const _HeroMetric({required this.label, required this.value, required this.data, this.green = false, this.purple = false, this.dense = false});
  final String label;
  final String value;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          const Spacer(),
          SizedBox(height: dense ? 14 : 22, width: double.infinity, child: CustomPaint(painter: _SparkPainter(data: data, color: color))),
        ],
      ),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 2),
            if (shown.isEmpty)
              SizedBox(height: dense ? 170 : 238, child: const Center(child: _EmptyState(icon: Icons.dns_outlined, title: '暂无节点', subtitle: '添加订阅或导入节点后会显示在这里。')))
            else
  ...
  Widget build(BuildContext context) => const Padding(
        ...
        child: Row(
          children: <Widget>[
            ...
            Expanded(flex: 2, child: Text('地区', style: TextStyle(fontSize: 9.5, color: Color(0xFF7B8798)))),
        ...
        child: AnimatedContainer(
          ...
          child: Row(
            children: <Widget>[
              ...
              Expanded(flex: 2, child: Text(_nodeRegionName(node), maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 9.2, color: Color(0xFF667085)))),
...
class _TrafficCard extends StatelessWidget {
  const _TrafficCard({required this.upload, required this.download, this.dense = false});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 10),
            SizedBox(height: dense ? 102 : 145, width: double.infinity, child: CustomPaint(painter: _TrafficPainter(upload: upload, download: download))),
            const SizedBox(height: 3),
            Row(
...
class _NodeRegionInfo {
  const _NodeRegionInfo(this.flag, this.name, this.code);
  final String flag;
  final String name;
  final String code;
}

_NodeRegionInfo _nodeRegion(NodeProfile node) {
  // Manual VLESS nodes are commonly named with an IP, arbitrary remark, or a
  // flag emoji. Region rendering must never be able to break the node row.
  // In particular, do not use an astral Unicode range such as [🇦-🇿] in a
  // non-unicode RegExp: on some Dart runtimes it can throw while the widget is
  // being built, leaving the dashboard/list apparently empty even though the
  // selected node is still available elsewhere in the UI.
  try {
    final raw = '${node.name} ${node.server}'.toLowerCase();
    final regions = <_NodeRegionInfo, RegExp>{
      const _NodeRegionInfo('🇭🇰', '香港', 'HK'): RegExp(r'香港|hong[ ._-]?kong|(?:^|[^a-z])hk(?:[^a-z]|$)|\.hk(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇯🇵', '日本', 'JP'): RegExp(r'日本|japan|tokyo|osaka|东京|大阪|(?:^|[^a-z])jp(?:[^a-z]|$)|\.jp(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇸🇬', '新加坡', 'SG'): RegExp(r'新加坡|singapore|(?:^|[^a-z])sg(?:[^a-z]|$)|\.sg(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇺🇸', '美国', 'US'): RegExp(r'美国|united[ ._-]?states|usa|los[ ._-]?angeles|san[ ._-]?jose|seattle|new[ ._-]?york|dallas|洛杉矶|圣何塞|西雅图|纽约|(?:^|[^a-z])us(?:[^a-z]|$)|\.us(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇹🇼', '台湾', 'TW'): RegExp(r'台湾|taiwan|taipei|台北|(?:^|[^a-z])tw(?:[^a-z]|$)|\.tw(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇰🇷', '韩国', 'KR'): RegExp(r'韩国|korea|seoul|首尔|(?:^|[^a-z])kr(?:[^a-z]|$)|\.kr(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇨🇳', '中国大陆', 'CN'): RegExp(r'中国大陆|china|beijing|shanghai|guangzhou|shenzhen|北京|上海|广州|深圳|(?:^|[^a-z])cn(?:[^a-z]|$)|\.cn(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇩🇪', '德国', 'DE'): RegExp(r'德国|germany|frankfurt|法兰克福|(?:^|[^a-z])de(?:[^a-z]|$)|\.de(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇬🇧', '英国', 'GB'): RegExp(r'英国|united[ ._-]?kingdom|london|伦敦|(?:^|[^a-z])(?:uk|gb)(?:[^a-z]|$)|\.uk(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇫🇷', '法国', 'FR'): RegExp(r'法国|france|paris|巴黎|(?:^|[^a-z])fr(?:[^a-z]|$)|\.fr(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇨🇦', '加拿大', 'CA'): RegExp(r'加拿大|canada|toronto|vancouver|多伦多|温哥华|(?:^|[^a-z])ca(?:[^a-z]|$)|\.ca(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇦🇺', '澳大利亚', 'AU'): RegExp(r'澳大利亚|澳洲|australia|sydney|melbourne|悉尼|墨尔本|(?:^|[^a-z])au(?:[^a-z]|$)|\.au(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇳🇱', '荷兰', 'NL'): RegExp(r'荷兰|netherlands|amsterdam|阿姆斯特丹|(?:^|[^a-z])nl(?:[^a-z]|$)|\.nl(?:\b|$)', caseSensitive: false),
      const _NodeRegionInfo('🇷🇺', '俄罗斯', 'RU'): RegExp(r'俄罗斯|russia|moscow|莫斯科|(?:^|[^a-z])ru(?:[^a-z]|$)|\.ru(?:\b|$)', caseSensitive: false),
    };
    for (final entry in regions.entries) {
      if (entry.value.hasMatch(raw)) return entry.key;
    }

    final flag = _extractFlagEmoji(node.name);
    if (flag != null) {
      const names = <String, String>{
        'HK': '香港', 'JP': '日本', 'SG': '新加坡', 'US': '美国',
        'TW': '台湾', 'KR': '韩国', 'CN': '中国大陆', 'DE': '德国',
        'GB': '英国', 'FR': '法国', 'CA': '加拿大', 'AU': '澳大利亚',
        'NL': '荷兰', 'RU': '俄罗斯',
      };
      return _NodeRegionInfo(flag.emoji, names[flag.code] ?? '其他地区', flag.code);
    }
  } catch (_) {
    // Country/flag detection is presentation-only. Never let it hide a valid
    // node or break a node list when a remark contains unusual Unicode.
  }
  return const _NodeRegionInfo('🌐', '未知地区', '--');
}

class _FlagEmoji {
  const _FlagEmoji(this.emoji, this.code);
  final String emoji;
  final String code;
}

_FlagEmoji? _extractFlagEmoji(String value) {
  ...
  for (var i = 0; i + 1 < codePoints.length; i++) {
    ...
    if (first < regionalBase || first > regionalLast || second < regionalBase || second > regionalLast) {
      continue;
    }
    ...
    return _FlagEmoji(String.fromCharCodes(<int>[first, second]), code);
  }
  return null;
}

String _nodeFlag(NodeProfile node) => _nodeRegion(node).flag;
String _nodeRegionName(NodeProfile node) => _nodeRegion(node).name;
        ...
        Expanded(
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                '${_nodeRegionName(node)} · ${node.protocol.toUpperCase()} · ${node.server}:${node.port}',
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: const TextStyle(
                  fontSize: 9.1,
    ...
    return Row(
      children: <Widget>[
        ...
        Expanded(
          flex: 2,
          child: Text(
            _nodeRegionName(node),
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
            style: const TextStyle(fontSize: 9.3, color: Color(0xFF53627A), fontWeight: FontWeight.w600),
          ),
        ),
        Expanded(
          flex: 2,
      ...
      children: <Widget>[
        _ToolbarCard(
          child: LayoutBuilder(builder: (context, constraints) {
            final info = const Row(children: <Widget>[
              Icon(Icons.lock_outline_rounded, size: 16, color: Color(0xFF1677FF)),
              SizedBox(width: 8),
              Expanded(child: Text('订阅地址仅保存在本机；更新时会替换该订阅上一次导入的节点。', style: TextStyle(fontSize: 10.8, color: Color(0xFF667085)))),
            ]);
            final actions = Wrap(spacing: 8, runSpacing: 8, children: <Widget>[
            ...
            if (constraints.maxWidth < 760) {
              return Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[info, const SizedBox(height: 9), actions]);
            }
...
class _NetworkToolsPage extends StatefulWidget {
  const _NetworkToolsPage({required this.controller, required this.settings, required this.nodes, required this.onNodesChanged});
  final SingBoxController controller;
  final AppSettings settings;
  final List<NodeProfile> nodes;
...
class _NetworkToolsPageState extends State<_NetworkToolsPage> {
  ...
  @override
  Widget build(BuildContext context) {
    final latencyCard = Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
      const _SectionHeader(title: '节点延迟 / 连通性'),
      const SizedBox(height: 8),
      Text(widget.controller.isRunning ? '已连接时使用 sing-box Clash API 做真实代理 URLTest；未连接时只检查 TCP 端口可达，不把 TCP 建连时间冒充代理延迟。' : '当前未连接：只检查 TCP 端口可达；连接后可得到真实代理 URLTest 延迟。', style: const TextStyle(color: Color(0xFF667085), fontSize: 11.5)),
      const SizedBox(height: 12),
    ...
    final ipCard = Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
    ...
    return ListView(children: <Widget>[
      LayoutBuilder(builder: (context, constraints) {
        if (constraints.maxWidth < 720) {
          return Column(children: <Widget>[latencyCard, const SizedBox(height: 10), ipCard]);
        }
        return Row(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[Expanded(child: latencyCard), const SizedBox(width: 12), Expanded(child: ipCard)]);
      }),
      const SizedBox(height: 12),
      Card(child: widget.nodes.isEmpty ? const Padding(padding: EdgeInsets.all(30), child: _EmptyState(icon: Icons.network_check, title: '暂无节点', subtitle: '添加节点后可进行批量延迟测试。')) : ListView.separated(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), padding: const EdgeInsets.all(14), itemCount: widget.nodes.length, separatorBuilder: (_, __) => const Divider(height: 1), itemBuilder: (context, index) {
        ...
        return ListTile(leading: Text(_nodeFlag(n), style: const TextStyle(fontSize: 18)), title: Text(n.name), subtitle: Text('${_nodeRegionName(n)} · ${n.server}:${n.port}'), trailing: Text(n.latencyMs == null ? '--' : '${n.latencyMs} ms', style: TextStyle(fontWeight: FontWeight.w700, color: n.latencyMs == null ? const Color(0xFF98A2B3) : _latencyColor(n.latencyMs!))));
      })),
    ]);
  }
        ...
        child: Column(
          children: <Widget>[
            LayoutBuilder(builder: (context, constraints) {
              ...
              final buttons = Wrap(spacing: 7, runSpacing: 7, children: <Widget>[
              ...
              if (compact) return Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[info, const SizedBox(height: 10), buttons]);
        ...
        child: Card(
          child: Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: <Widget>[
              ...
              const SizedBox(height: 5),
              const Text('V1.6.5 · Windows x64', style: TextStyle(color: Color(0xFF667085))),
