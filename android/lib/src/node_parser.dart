...
class NodeParser {
  ...
  static List<NodeProfile> parseMany(String content, {String source = 'manual'}) {
  ...
  static NodeProfile _parseVless(Uri uri, String source) {
    ...
    _applyTlsAndTransport(outbound, q, defaultTls: q['security'] == 'tls' || q['security'] == 'reality');
  ...
  static NodeProfile _parseHysteria2(Uri uri, String source) {
    ...
    if (obfs != null && obfs.isNotEmpty) {
      outbound['obfs'] = <String, dynamic>{
        'type': obfs,
        if (obfsPassword != null && obfsPassword.isNotEmpty) 'password': obfsPassword,
      };
    }
  ...
  static NodeProfile _parseSocks(Uri uri, String source) {
    ...
    if (uri.userInfo.isNotEmpty) {
      ...
      if (parts.length > 1) outbound['password'] = Uri.decodeComponent(parts.sublist(1).join(':'));
    }
  ...
  static NodeProfile? _parseVmess(String input, String source) {
    try {
      ...
      final outbound = <String, dynamic>{
        ...
        'security': (json['scy']?.toString() ?? '').isNotEmpty ? json['scy'].toString() : 'auto',
      ...
      return NodeProfile(
        id: _id(),
        name: (json['ps']?.toString() ?? '').isNotEmpty ? json['ps'].toString() : '$server:$port',
        protocol: 'vmess',
        server: server,
        port: port,
        outbound: outbound,
  ...
  static NodeProfile _fromOutbound(Map<String, dynamic> outbound, {required String source}) {
    ...
    return NodeProfile(
      id: _id(),
      name: (outbound['tag']?.toString() ?? '').isNotEmpty ? outbound['tag'].toString() : '$type $server:$port',
      protocol: type,
      server: server,
      port: port,
  ...
  static void _applyTlsAndTransport(
    ...
    if (type == 'ws') {
      outbound['transport'] = <String, dynamic>{
        ...
        if ((q['host'] ?? '').isNotEmpty) 'headers': <String, String>{'Host': q['host']!},
      };
    } else if (type == 'grpc') {
      outbound['transport'] = <String, dynamic>{
        'type': 'grpc',
  ...
  static void _applyTls(
    ...
    final insecure = q['allowInsecure'] == '1' || q['insecure'] == '1' || q['insecure'] == 'true';
    ...
    if (security == 'reality' || (pbk != null && pbk.isNotEmpty)) {
      tls['reality'] = <String, dynamic>{
        ...
        if ((q['sid'] ?? q['shortId'] ?? '').isNotEmpty) 'short_id': q['sid'] ?? q['shortId'],
      };
    }
    outbound['tls'] = tls;
  }
